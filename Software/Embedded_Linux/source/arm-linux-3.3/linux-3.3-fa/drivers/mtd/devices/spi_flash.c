...
static struct mtd_partition ftspi020_partition_info[11];        //10 + 1 total MTD
...
static int wait_till_ready(struct common_spi_flash *flash, int wait_time)
{
	...
	if (sr & SR_WIP)
        printk("SPI wait ready time out\n");
	...
	if (addr != 0 || len != mtd->size) {
		while (len) {
        	...
        	if (wait_till_ready(flash, 5)) {
        	    printk("SPI wait previous erase command time out\n");
        		mutex_unlock(&flash->lock);
        		return -EIO;
        	}			
		}
        ...
        if (wait_till_ready(flash, 500)) {
            printk("SPI wait chip erase command time out\n");
        	mutex_unlock(&flash->lock);
        	return -EIO;
        }		
    }
                                  ...
                                  size_t * retlen, const u_char * buf)
{
    ...
    /* Wait until finished previous write command. */
	if (wait_till_ready(flash, 3)) {
	    printk("SPI wait previous command time out\n");
        mutex_unlock(&flash->lock);
        return 1;
    }
...
static struct flash_info __devinitdata common_spi_flash_data[] = {
    ...
    /* esmt */
    {"w25Q64F", 0x8c4117, 0, 65536, 128, SECT_64K, 3, 1},
	/* Spansion */
	{"s25fl164k", 0x014017, 0, 65536, 128, SECT_64K, 3, 1},
	{"s25fl128s", 0x012018, 0, 65536, 256, SECT_64K, 3, 1},
	{"s25fl256s", 0x010219, 0, 65536, 512, SECT_64K, 4, 1},
	/* GD */
    {"gd25q64c", 0xc84017, 0, 65536, 128, SECT_64K, 3, 1},	
	{"gd25q128c", 0xc84018, 0, 65536, 256, SECT_64K, 3, 1},
	{"gd25q256c", 0xc84019, 0, 65536, 512, SECT_64K, 4, 1},
#else
    /* winbond */
...
static struct flash_info *__devinit jedec_probe(struct spi_device *spi)
{
    ...
    if (m.status < 0) {
        printk("%s: error %d reading JEDEC ID\n", dev_name(&spi->dev), m.actual_length);
        return NULL;
    }
...
static int partition_check(struct mtd_partition *partitions, sys_header_t *sys_hdr, int page_size, int chip_size)
{
    ...
    for(i = 0; i < ARRAY_SIZE(ftspi020_partition_info) - 1; i++){
        
        if((sys_hdr->image[i].addr == 0) || (sys_hdr->image[i].size == 0))
    	    continue;
...
static int __devinit common_spi_flash_probe(struct spi_device *spi)
{
    ...
    if (flash->mode_3_4 == 3){
        ...
        switch (flash->flash_type){//1:winbond, status register is different, 2:MXIC, 3:EON
            case 1:
                OPCODE_READ_COMMAND = 0x0B;
                OPCODE_WRITE_COMMAND = 0x02;
                OPCODE_ERASE_COMMAND = 0xD8;
            ...
            case 2:
                ...
                if (platform_spi_four_byte_mode()) {
                    OPCODE_READ_COMMAND = 0x0B;
                    OPCODE_WRITE_COMMAND = 0x02;
                    OPCODE_ERASE_COMMAND = 0xD8;
                } else {
