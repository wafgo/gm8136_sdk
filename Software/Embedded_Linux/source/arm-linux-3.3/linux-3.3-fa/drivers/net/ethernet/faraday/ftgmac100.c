...
static struct proc_dir_entry *ftgmac_rx_pkg_debug = NULL, *ftgmac_phy_debug = NULL, *ftgmac_rx_debug = NULL;
...
static bool ftgmac100_txdes_last_segment(struct ftgmac100_txdes *txdes)
{
...
static bool ftgmac100_txdes_used(struct ftgmac100_txdes *txdes)
{
	return txdes->txdes1 & cpu_to_le32(FTGMAC100_TXDES1_USED); 
}
...
static bool ftgmac100_clean_tx_ring(struct ftgmac100_tx_ring *tx_ring, int *nr_frag)
{
	...
	if (ftgmac100_txdes_last_segment(txdes)) {
		netdev->stats.tx_packets++;
		netdev->stats.tx_bytes += skb->len;
		dev_kfree_skb(skb);

		netif_wake_subqueue(netdev, tx_ring->tx_queue_index);
	}
...
static void ftgmac100_tx_complete(struct ftgmac100 *priv)
{
	int i, nr_frag = 0;
	for (i = 0; i < priv->tx_num_queues; i++) {
		while(ftgmac100_clean_tx_ring(&priv->tx_rings[i], &nr_frag))
			;
	}
}
 ...
 */
static int ftgmac100_reserve_txdeses(struct ftgmac100_tx_ring *tx_ring, int num)
{
...
static int ftgmac100_xmit(struct ftgmac100 *priv, struct sk_buff *skb)
{
	...
	txdes_idx = ftgmac100_reserve_txdeses(tx_ring, num_txdeses);

	if (unlikely(txdes_idx < 0)) {
		netif_stop_subqueue(netdev, tx_ring->tx_queue_index);
					...
					const struct ftgmac100_tx_reg_offset *reg_offset)
{
	...
	tx_ring->txdes = dma_alloc_coherent(tx_ring->dev, 
										...
										&tx_ring->tx_dma_addr, GFP_KERNEL);
	if (!tx_ring->txdes) {
		if (net_ratelimit())
				...
				const struct ftgmac100_rx_reg_offset *reg_offset)
{
	...
	rx_ring->rxdes = dma_alloc_coherent(rx_ring->dev, 
										...
										&rx_ring->rx_dma_addr, GFP_KERNEL);
	...
	for (i = 0; i < rx_ring->rx_num_entries; i++) {
		...
		if (ftgmac100_alloc_rx_page(rx_ring, rxdes, GFP_KERNEL | GFP_DMA))
			return -ENOMEM;
    }
...
static void ftgmac100_free_txdes(struct ftgmac100_tx_ring *tx_ring)
{
	int nr_frag = 0;
	if (tx_ring->txdes) {
        while(ftgmac100_clean_tx_ring(tx_ring, &nr_frag));
...
static void ftgmac100_free_buffers(struct ftgmac100 *priv)
{
	...
	if (priv->tx_rings) {
		for (i = 0; i< priv->tx_num_queues; i++) {
			ftgmac100_free_txdes(&priv->tx_rings[i]);
		}
		vfree(priv->tx_rings);
	}
}
...
static int ftgmac100_set_tfifo_size(struct ftgmac100 *priv, int size_sel)
{
	...
	if (size_sel > max_sel) {
		...
		tpafcr |= FTGMAC100_TPAFCR_TFIFO_SIZE(size_sel);
		iowrite32(tpafcr, priv->base + FTGMAC100_OFFSET_TPAFCR);
		return size_sel;
	}
}
...
static int ftgmac100_set_rfifo_size(struct ftgmac100 *priv, int size_sel)
{
	...
	if (size_sel > max_sel) {
		...
		tpafcr |= FTGMAC100_TPAFCR_RFIFO_SIZE(size_sel);
		iowrite32(tpafcr, priv->base + FTGMAC100_OFFSET_TPAFCR);
		return size_sel;
	}
}
...
static int ftgmac100_probe(struct platform_device *pdev)
{
	...
	netdev->features = NETIF_F_IP_CSUM | NETIF_F_GRO | NETIF_F_GSO | NETIF_F_SG;
