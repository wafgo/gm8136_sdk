...
#if defined(CONFIG_PLATFORM_GM8139) || defined(CONFIG_PLATFORM_GM8287) || defined(CONFIG_PLATFORM_GM8136)
#define AHBMASTER_R_SRC   FTDMA020_AHBMASTER_0
...
static void ftspi020_dma_enable(void __iomem * base)
{
	...
	printk("FTSPI020 enable DMA handshake 0x%x\n", inl(base + FTSPI020_REG_ICR));
}
...
static int ftspi020_dma_wait(void)
{
    ...
    rc = wait_event_timeout(spi020_queue, trigger_flag == 1, 10 * HZ);
    if (rc == 0) {
    	printk("spi020 dma queue wake up timeout signal arrived\n");
    	return -1;
    }
				       ...
				       struct spi_transfer *t, int wsize)
{
	...
	if (cmd->flags & FTSPI020_XFER_CHECK_CMD_COMPLETE) {
		...
		len = wait_event_timeout(ctrl->waitq, g_cmd_complete, 10 * HZ);
		if (len == 0) {
    	    printk("spi020 complete queue wake up timeout signal arrived\n");
    	    return -1;
        }
	}
#if defined(CONFIG_FTSPI020_USE_AHBDMA) || defined(CONFIG_FTSPI020_USE_AXIDMA)
...
static int ftspi020_ctrl_master_setup(struct spi_device *spi)
{
    ...
    printk("CLK div field set %d, clock = %dHz\n", CLK_DIV, CLK);
...
static int ftspi020_probe(struct platform_device *pdev)
{
...
#if defined(CONFIG_FTSPI020_USE_AHBDMA) || defined(CONFIG_FTSPI020_USE_AXIDMA)
#ifdef CONFIG_FTSPI020_USE_AHBDMA
    printk("SPI020 uses AHB DMA mode\n");
#else
    ftpmu010_write_reg(spi020_fd, 0xA4, (0x0 << 26), (0x1 << 26));
    printk("SPI020 uses AXI DMA mode\n");
#endif
    ...
    printk("SPI020 gets DMA channel %d\n", ctrl->dma_chan->chan_id);
	...
	printk("Probe FTSPI020 SPI Controller at 0x%08x (irq %d)\n", (unsigned int)res->start, irq);
	return 0;
...
static int __init ftspi020_init(void)
{
   ...
   if (spi020_fd < 0)
       printk("SPI020: register PMU fail");
