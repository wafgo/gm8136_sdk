...
static inline int ftssp010_set_speed(struct ftssp010_spi *ftssp010_spi, u32 speed_hz)
{
    ...
    if(scldiv > 0xFFFF)
        printk("SSP speed too fast, can't be div to %dHz\n",speed_hz);
                                       ...
                                       struct spi_device *spi, struct spi_transfer *t, int wsize)
{
    ...
    while (len > 0) {
        ...
        for (i = 0; i < count; i++) {
            ...
            if(j >= 0x1000000)
                printk("wait ftssp010_rxfifo_valid_entries timeout\n");
...
static int ftssp010_spi_probe(struct platform_device *pdev)
{
    ...
    if (unlikely(spi_hw_platform == NULL)) {
        printk("Error => %s: SSP(%d) spi_hw_platform is NULL.\n", __func__, pdev->id);
    }
...
*/    
    ...
    printk("bus num = %d, chip select = %d\n",master->bus_num,master->num_chipselect);
