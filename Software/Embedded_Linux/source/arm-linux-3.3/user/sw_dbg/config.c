...
static int getsection(const char *line, char *section,
		      unsigned int seclen);
static int getkeyname(const char *line, char *keyname,
		      unsigned int keylen);
static int getkeyvalue(const char *line, char *keyvalue,
		       unsigned int valen);
...
int getconfigstr(const char *section,
		 const char *keyname,
		 char *keyvalue, unsigned int len, const char *filename)
{
    int step = 0;
    int rs = 0;
    int ra = 0;
...
int getconfigint(const char *section,
		 const char *keyname, int *keyvalue, const char *filename)
{
    int rs;
    char kev[12];
    ...
    if (rs == 0)
	    *keyvalue = strtol(kev,NULL,0);
    return rs;
}

int getconfiguint(const char *section,
		 const char *keyname, unsigned int *keyvalue, const char *filename)
{
    int rs;
    char kev[12];
