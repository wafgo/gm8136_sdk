...
_XDATA BYTE bOutputAudioMode = 0x00 ; // use full packet as default 
...
//  {  31 ,0x06, 0xff, 0x0e, 0x3a,0x22, 0x04, 0x90, 0xFF, 0xFF, 0x00      ,0xFF          ,0xFF      ,0xFF          ,0xFF	        ,148500000,50},
    ...
    {  34 ,0xF6, 0xFF, 0x56, 0x82,0x00, 0x04, 0x90, 0xFF, 0xFF, 0xF6      ,0xFF          ,0xAD      ,0x05          ,0x10            , 74000000,30},
     ...
     /* 1024x768P */
     {36   ,0xf0, 0xff,0x16,0x9E,0x00,0x03,0x60,0xFF,0xFF,0xFF      ,0xFF          ,0xFF      ,0xFF          ,0xFF            , 74000000,60},
     /* 1280x1024P */
     {37   ,0xf6,0xff, 0x2E,0x9E,0x00,0x01,0x40,0xFF,0xFF,0xFF      ,0xFF          ,0xFF      ,0xFF          ,0xFF            , 74000000,60},
     // 1680x1050P, HDES = 110, HDEE = 1790, VDES = 26, VDEE = 1076
     {38   ,0xF4,0xFF, 0x66,0x16,0x10, 0x3,0x90, 0xFF,0xFF,0xFF      ,0xFF          ,0xFF      ,0xFF          ,0xFF            ,148000000,60},
     // HDES = 454, HDEE = 2134, VDES = 35, VDEE = 1085     
     {39   ,0xF6,0xFF, 0x40,0x00,0x10, 0x01,0x40, 0xFF,0xFF,0xFF      ,0xFF          ,0xFF      ,0xFF          ,0xFF            ,148000000,60},
     {40   ,0xf4,0xFF, 0x4E,0xE6,0x00,  0x03,0x90, 0xFF,0xFF,0xFF      ,0xFF          ,0xFF      ,0xFF          ,0xFF            ,106500000,60},
     // HDES = 454, HDEE = 2134, VDES = 35, VDEE = 1085
...
EnableAudioOutput(unsigned long VideoPixelClock, unsigned long AudioSampleClock, int AudioChannel, BOOL bSPDIF)
{
    ...
    SetAudioFormat(AudioChannel,bAudioChannelEnable,AudioSampleClock,16,0) ;
...
//    BYTE uc ;
    ...
    //HDMITX_WriteI2C_Byte(REG_CLK_CTRL1,HDMITX_ReadI2C_Byte(REG_CLK_CTRL1)|0x08);
...
ReadEDID(BYTE *pData, BYTE bSegment, BYTE offset, SHORT Count)
{
    ...
    while( RemainedCount > 0 )
    {
        ...
        if( TimeOut == 0 )
        {
            ErrorF("ReadEDID(): DDC TimeOut. %02X \n",ucdata) ;
            return ER_FAIL ;
        }
...
HDCP_GetVr(BYTE *pVr)
{
    ...
    if(TimeOut == 0)
    {
        ErrorF("HDCP_GetVr(): DDC fail by timeout. %x\n",ucdata) ;
        return ER_FAIL ;
    }
