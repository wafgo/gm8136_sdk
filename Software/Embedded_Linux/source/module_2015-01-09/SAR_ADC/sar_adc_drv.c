 ...
 * VER1.9 2014/10/29 MOVE fifo fail msg to debug state
 */
...
int init_hardware(struct dev_data *p_dev_data)
{
    ...
    if(!run_mode)
        p_dev_data->running_mode = SAR_ADC_RUNING_MODE_ALL;
    else
...
#endif
{
    ...
    /*print runmod and gain0 min and max value*/
     printk("%s done,runmod:0x%x,gain0 min-max(0x%x-0x%x)\n", 
    __FUNCTION__, 
    (unsigned int)run_mode, 
    (unsigned int)gain0_lim[0]&0xff, 
    (unsigned int)gain0_lim[1]&0xff
    );
    return ret;
