...
#undef PFX
#define PFX 					"ADDA300-0"


#ifdef GM8312_SSP_TEST
/*static enum adda300_port_select cs = ADDA300_AD;
...
#if SSP3_CLK_SRC_12000K
    {400000, 800000, 1499400, 1600000, 2998800, 2400000};
#else
    {400000, 800000, 1499400, 1600000, 2998800, 3072000};   // ssp3 clk source = 24Mhz for sample rate 48k, bclk = 3072000, the other sample rate's bclk need modify!!
#endif
...
static int fi2s_adda300_init(void)
{
    ...
    card = ftssp010_ssp_probe(3, THIS_MODULE);
    if (IS_ERR(card)) {
        ret = PTR_ERR(card);
        goto exit_err2;
    }
    ...
    adda300_init(cs);
#ifdef GM8312_SSP_TEST
    adda300_I2S_mux(cs, ADDA300_FROM_PAD);
    adda300_I2S_mux(ADDA300_DA, ADDA300_FROM_PAD);
#endif
