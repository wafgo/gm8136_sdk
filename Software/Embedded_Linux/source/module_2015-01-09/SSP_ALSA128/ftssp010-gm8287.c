/**
 * @file ftssp010-gm8287.c
 * Grain Media's SSP PMU settings of GM8287 series SOC
 *
 *
 */
...
/* reg_off,  bit_masks,   lock_bits,     init_val,   init_mask */
#if CLK_SRC_12000K
    {0x28,  0x3F << 20,   0,    0x15 << 20,   0x3F << 20},  //SSP0-2 select 12M as the source
    {0xb8,  0x7 << 6,     0  ,     0x0,          0x0},         //SSP0-2 pclk & sclk
    ...
    {0xb8,  0x7 << 6,    0x7 << 6,      0x0,          0x0},         //SSP0-2 pclk & sclk
#else   // SSP0~2/4~5 as 12MHz, SSP3 as 24MHz
    {0x28,  0x3F << 20,  0x3F << 20,    0x5 << 20,    0x3F << 20}, //SSP0-1 select 12M as the source, SSP2 select clk_12288 as source
    {0x60,  ((0x3 << 12) | (0x3FF << 17) | (0x1 << 16)),   (0x3 << 12) | (0x3FF << 17) | (0x1 << 16),  0x0,  (0x3 << 12) | (0x3FF << 17) | (0x1 << 16)},
    {0xb8,  0x7 << 6,    0x7 << 6 ,     0x0,          0x0},         //SSP0-2 pclk & sclk
#endif
};
...
static int pmu_init(ftssp010_card_t *card)
{
    ...
    /* totally we have 3 ssps */
    if ((card->cardno < 0) || (card->cardno > 2)) {
