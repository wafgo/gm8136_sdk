/*
***********************************************************************************
** FARADAY(MTD)/GRAIN CONFIDENTIAL
** Copyright FARADAY(MTD)/GRAIN Corporation All Rights Reserved.
**
** The source code contained or described herein and all documents related to
** the source code (Material) are owned by FARADAY(MTD)/GRAIN Corporation Title to the
** Material remains with FARADAY(MTD)/GRAIN Corporation. The Material contains trade
** secrets and proprietary and confidential information of FARADAY(MTD)/GRAIN. The
** Material is protected by worldwide copyright and trade secret laws and treaty
** provisions. No part of the Material may be used, copied, reproduced, modified,
** published, uploaded, posted, transmitted, distributed, or disclosed in any way
** without FARADAY(MTD)/GRAIN prior express written permission.
**
** No license under any patent, copyright, trade secret or other intellectual
** property right is granted to or conferred upon you by disclosure or delivery
** of the Materials, either expressly, by implication, inducement, estoppel or
** otherwise. Any license under such intellectual property rights must be express
** and approved by FARADAY(MTD)/GRAIN in writing.
**
**
***********************************************************************/


/*******************************************************************************
	File:		MDCT.c

	Content:	MDCT Transform functionss

*******************************************************************************/

#include "basic_op.h"
#include "perceptual_const.h"
#include "enc_mdct.h"
#include "aac_table.h"


#define LS_TRANS ((FRAME_LEN_LONG-FRAME_LEN_SHORT)/2) /* 448 */
#define SQRT1_2 0x5a82799a	/* sqrt(1/2) in Q31 */
#define swap2(p0,p1) \
	t = p0; t1 = *(&(p0)+1);	\
	p0 = p1; *(&(p0)+1) = *(&(p1)+1);	\
	p1 = t; *(&(p1)+1) = t1

/*********************************************************************************
*
* function name: ShuffleSwap
* description:  ShuffleSwap points prepared function for fft
*
**********************************************************************************/
static void ShuffleSwap(int *buf, int num, const unsigned char* bitTab)
{
    int *part0, *part1;
	int i, j;
	int t, t1;

	part0 = buf;
    part1 = buf + num;

	while ((i = *bitTab++) != 0) {
        j = *bitTab++;

        swap2(part0[4*i+0], part0[4*j+0]);
        swap2(part0[4*i+2], part1[4*j+0]);
        swap2(part1[4*i+0], part0[4*j+2]);
        swap2(part1[4*i+2], part1[4*j+2]);
    }

    do {
        swap2(part0[4*i+2], part1[4*i+0]);
    } while ((i = *bitTab++) != 0);
}

#if !defined(ARMV5E) && !defined(ARMV7Neon)

/*****************************************************************************
*
* function name: Radix4_1
* description:  Radix 4 point prepared function for fft
*
**********************************************************************************/
static void Radix4_1(int *buf, int num)
{
    int r0, r1, r2, r3;
	int r4, r5, r6, r7;

//	for (; num != 0; num--)
	do {
		r0 = buf[0] + buf[2];
		r1 = buf[1] + buf[3];
		r2 = buf[0] - buf[2];
		r3 = buf[1] - buf[3];
		r4 = buf[4] + buf[6];
		r5 = buf[5] + buf[7];
		r6 = buf[4] - buf[6];
		r7 = buf[5] - buf[7];

		buf[0] = r0 + r4;
		buf[1] = r1 + r5;
		buf[4] = r0 - r4;
		buf[5] = r1 - r5;
		buf[2] = r2 + r7;
		buf[3] = r3 - r6;
		buf[6] = r2 - r7;
		buf[7] = r3 + r6;

		buf += 8;
	} while (--num!=0);
}

/*****************************************************************************
*
* function name: Radix8FFT
* description:  Radix 8 point prepared function for fft
*
**********************************************************************************/
static void Radix8FFT(int *buf, int num)
{
   int r0, r1, r2, r3;
   int i0, i1, i2, i3;
   int r4, r5, r6, r7;
   int i4, i5, i6, i7;
   int t0, t1, t2, t3;
   int tmp = 0, tmp1 = 0;
//	for ( ; num != 0; num--)
	do {
		r0 = buf[0] + buf[2];
		i0 = buf[1] + buf[3];
		r1 = buf[0] - buf[2];
		i1 = buf[1] - buf[3];
		r2 = buf[4] + buf[6];
		i2 = buf[5] + buf[7];
		r3 = buf[4] - buf[6];
		i3 = buf[5] - buf[7];

		r4 = (r0 + r2) >> 1;
		i4 = (i0 + i2) >> 1;
		r5 = (r0 - r2) >> 1;
		i5 = (i0 - i2) >> 1;
		r6 = (r1 - i3) >> 1;
		i6 = (i1 + r3) >> 1;
		r7 = (r1 + i3) >> 1;
		i7 = (i1 - r3) >> 1;

		r0 = buf[ 8] + buf[10];
		i0 = buf[ 9] + buf[11];
		r1 = buf[ 8] - buf[10];
		i1 = buf[ 9] - buf[11];
		r2 = buf[12] + buf[14];
		i2 = buf[13] + buf[15];
		r3 = buf[12] - buf[14];
		i3 = buf[13] - buf[15];

		t0 = (r0 + r2) >> 1;
		t1 = (i0 + i2) >> 1;
		t2 = (r0 - r2) >> 1;
		t3 = (i0 - i2) >> 1;

		buf[ 0] = r4 + t0;
		buf[ 1] = i4 + t1;
		buf[ 8] = r4 - t0;
		buf[ 9] = i4 - t1;
		buf[ 4] = r5 + t3;
		buf[ 5] = i5 - t2;
		buf[12] = r5 - t3;
		buf[13] = i5 + t2;

		r0 = r1 - i3;
		i0 = i1 + r3;
		r2 = r1 + i3;
		i2 = i1 - r3;
#ifdef LINUX
		tmp1 = r0 - i0;
		asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (tmp), [o2] "+r" (t0) : [i1] "r" (SQRT1_2), [i2] "r" (tmp1));
		tmp1 = r0 + i0;
		asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (tmp), [o2] "+r" (t1) : [i1] "r" (SQRT1_2), [i2] "r" (tmp1));
		tmp1 = r2 - i2;
		asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (tmp), [o2] "+r" (t2) : [i1] "r" (SQRT1_2), [i2] "r" (tmp1));
		tmp1 = r2 + i2;
		asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (tmp), [o2] "+r" (t3) : [i1] "r" (SQRT1_2), [i2] "r" (tmp1));
#else
		tmp1 = r0 - i0;
		__asm {
			SMULL	tmp, t0, SQRT1_2, tmp1
		}
		tmp1 = r0 + i0;
		__asm {
			SMULL	tmp, t1, SQRT1_2, tmp1
		}
		tmp1 = r2 - i2;
		__asm {
			SMULL	tmp, t2, SQRT1_2, tmp1
		}
		tmp1 = r2 + i2;
		__asm {
			SMULL	tmp, t3, SQRT1_2, tmp1
		}
#endif
//		t0 = MULHIGH(SQRT1_2, r0 - i0);
//		t1 = MULHIGH(SQRT1_2, r0 + i0);
//		t2 = MULHIGH(SQRT1_2, r2 - i2);
//		t3 = MULHIGH(SQRT1_2, r2 + i2);

		buf[ 6] = r6 - t0;
		buf[ 7] = i6 - t1;
		buf[14] = r6 + t0;
		buf[15] = i6 + t1;
		buf[ 2] = r7 + t3;
		buf[ 3] = i7 - t2;
		buf[10] = r7 - t3;
		buf[11] = i7 + t2;

		buf += 16;
	} while (--num!=0);
}

/*****************************************************************************
*
* function name: Radix4FFT
* description:  Radix 4 point fft core function
*
**********************************************************************************/
static void Radix4FFT(int *buf, int num, int bgn, int *twidTab)
{
	int r0, r1, r2, r3;
	int r4, r5, r6, r7;
	int t0, t1;
	int sinx, cosx;
	int i, j, fftstep;
	int *xptr, *csptr;
	int acc0 = 0, acc1 = 0, acc2 = 0;

	for (num >>= 2; num != 0; num >>= 2)
	{
		fftstep = 2*bgn;
		xptr = buf;
		i = num;
//    	for (i = num; i != 0; i--)
		do {
			csptr = twidTab;
			j = bgn;
//			for (j = bgn; j != 0; j--)
			do {

#ifdef LINUX
				r0 = xptr[0];
				r1 = xptr[1];
				xptr += fftstep;

				t0 = xptr[0];
				t1 = xptr[1];
				cosx = *csptr++;
				sinx = *csptr++;
				asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (cosx), [i2] "r" (t0));
				asm volatile("smlal %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (sinx), [i2] "r" (t1));
				r2 = acc0;
				asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (cosx), [i2] "r" (t1));
				asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc1) : [i1] "r" (sinx), [i2] "r" (t0));
				r3 = acc0 - acc1;
				xptr += fftstep;

				t0 = r0 >> 2;
				t1 = r1 >> 2;
				r0 = t0 - r2;
				r1 = t1 - r3;
				r2 = t0 + r2;
				r3 = t1 + r3;

				t0 = xptr[0];
				t1 = xptr[1];
				cosx = *csptr++;
				sinx = *csptr++;
				asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (cosx), [i2] "r" (t0));
				asm volatile("smlal %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (sinx), [i2] "r" (t1));
				r4 = acc0;
				asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (cosx), [i2] "r" (t1));
				asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc1) : [i1] "r" (sinx), [i2] "r" (t0));

				r5 = acc0 - acc1;
				xptr += fftstep;

				t0 = xptr[0];
				t1 = xptr[1];
				cosx = *csptr++;
				sinx = *csptr++;
				asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (cosx), [i2] "r" (t0));
				asm volatile("smlal %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (sinx), [i2] "r" (t1));
				r6 = acc0;
				asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (cosx), [i2] "r" (t1));
				asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc1) : [i1] "r" (sinx), [i2] "r" (t0));
				r7 = acc0 - acc1;
				t0 = r4;
				t1 = r5;
				r4 = t0 + r6;
				r5 = r7 - t1;
				r6 = t0 - r6;
				r7 = r7 + t1;

				xptr[0] = r0 + r5;
				xptr[1] = r1 + r6;
				xptr -= fftstep;

				xptr[0] = r2 - r4;
				xptr[1] = r3 - r7;
				xptr -= fftstep;

				xptr[0] = r0 - r5;
				xptr[1] = r1 - r6;
				xptr -= fftstep;

				xptr[0] = r2 + r4;
				xptr[1] = r3 + r7;
				xptr += 2;
#else
				r0 = xptr[0];
				r1 = xptr[1];
				xptr += fftstep;

				t0 = xptr[0];
				t1 = xptr[1];
				cosx = csptr[0];
				sinx = csptr[1];
				__asm {
					SMULL	acc2, acc0, cosx, t0
//					MOV		acc2, #0
					SMLAL	acc2, acc0, sinx, t1
				}
				r2 = acc0;
				__asm {
					SMULL	acc2, acc0, cosx, t1
					SMULL	acc2, acc1, sinx, t0
				}
				r3 = acc0 - acc1;
//				r2 = MULHIGH(cosx, t0) + MULHIGH(sinx, t1);		/* cos*br + sin*bi */
//				r3 = MULHIGH(cosx, t1) - MULHIGH(sinx, t0);		/* cos*bi - sin*br */
				xptr += fftstep;

				t0 = r0 >> 2;
				t1 = r1 >> 2;
				r0 = t0 - r2;
				r1 = t1 - r3;
				r2 = t0 + r2;
				r3 = t1 + r3;

				t0 = xptr[0];
				t1 = xptr[1];
				cosx = csptr[2];
				sinx = csptr[3];
				__asm {
					SMULL	acc2, acc0, cosx, t0
//					MOV		acc2, #0
					SMLAL	acc2, acc0, sinx, t1
				}
				r4 = acc0;
				__asm {
					SMULL	acc2, acc0, cosx, t1
					SMULL	acc2, acc1, sinx, t0
				}
				r5 = acc0 - acc1;
//				r4 = MULHIGH(cosx, t0) + MULHIGH(sinx, t1);		/* cos*cr + sin*ci */
//				r5 = MULHIGH(cosx, t1) - MULHIGH(sinx, t0);		/* cos*ci - sin*cr */
				xptr += fftstep;

				t0 = xptr[0];
				t1 = xptr[1];
				cosx = csptr[4];
				sinx = csptr[5];
				__asm {
					SMULL	acc2, acc0, cosx, t0
//					MOV		acc2, #0
					SMLAL	acc2, acc0, sinx, t1
				}
				r6 = acc0;
				__asm {
					SMULL	acc2, acc0, cosx, t1
					SMULL	acc2, acc1, sinx, t0
				}
				r7 = acc0 - acc1;
//				r6 = MULHIGH(cosx, t0) + MULHIGH(sinx, t1);		/* cos*cr + sin*ci */
//				r7 = MULHIGH(cosx, t1) - MULHIGH(sinx, t0);		/* cos*ci - sin*cr */
				csptr += 6;

				t0 = r4;
				t1 = r5;
				r4 = t0 + r6;
				r5 = r7 - t1;
				r6 = t0 - r6;
				r7 = r7 + t1;

				xptr[0] = r0 + r5;
				xptr[1] = r1 + r6;
				xptr -= fftstep;

				xptr[0] = r2 - r4;
				xptr[1] = r3 - r7;
				xptr -= fftstep;

				xptr[0] = r0 - r5;
				xptr[1] = r1 - r6;
				xptr -= fftstep;

				xptr[0] = r2 + r4;
				xptr[1] = r3 + r7;
				xptr += 2;
#endif
			} while (--j!=0);
			xptr += 3*fftstep;
		} while (--i!=0);
		twidTab += 3*fftstep;
		bgn <<= 2;
	}
}

/*********************************************************************************
*
* function name: MDCT_One
* description:  prepare MDCT process for next FFT compute
*
**********************************************************************************/
static void MDCT_One(int *buf0, int num, const int *csptr)
{
	int i;
	int tr1, ti1, tr2, ti2;
	int cosa, sina, cosb, sinb;
	int *buf1;
	int acc0 = 0, acc1 = 0, acc2 = 0;

	buf1 = buf0 + num - 1;
	i = num >> 2;
//	for(i = num >> 2; i != 0; i--)
	do {
		cosa = *csptr++;
		sina = *csptr++;
		cosb = *csptr++;
		sinb = *csptr++;

		tr1 = *(buf0 + 0);
		ti2 = *(buf0 + 1);
		tr2 = *(buf1 - 1);
		ti1 = *(buf1 + 0);
#ifdef LINUX
		asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (cosa), [i2] "r" (tr1));
		asm volatile("smlal %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (sina), [i2] "r" (ti1));
		*buf0++ = acc0;
		asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (cosa), [i2] "r" (ti1));
		asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc1) : [i1] "r" (sina), [i2] "r" (tr1));
		*buf0++ = acc0 - acc1;
		asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (cosb), [i2] "r" (ti2));
		asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc1) : [i1] "r" (sinb), [i2] "r" (tr2));
		*buf1-- = acc0 - acc1;
		asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (cosb), [i2] "r" (tr2));
		asm volatile("smlal %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (sinb), [i2] "r" (ti2));
#else
		__asm {
			SMULL	acc2, acc0, cosa, tr1
//			MOV		acc2, #0
			SMLAL	acc2, acc0, sina, ti1
		}
		*buf0++ = acc0;
		__asm {
			SMULL	acc2, acc0, cosa, ti1
			SMULL	acc2, acc1, sina, tr1
		}
		*buf0++ = acc0 - acc1;

		__asm {
			SMULL	acc2, acc0, cosb, ti2
			SMULL	acc2, acc1, sinb, tr2
		}
		*buf1-- = acc0 - acc1;
		__asm {
			SMULL	acc2, acc0, cosb, tr2
//			MOV		acc2, #0
			SMLAL	acc2, acc0, sinb, ti2
		}
#endif
		*buf1-- = acc0;
//		*buf1-- = MULHIGH(cosb, ti2) - MULHIGH(sinb, tr2);
//		*buf1-- = MULHIGH(cosb, tr2) + MULHIGH(sinb, ti2);
	} while (--i!=0);
}

/*********************************************************************************
*
* function name: MDCT_Second
* description:   post MDCT process after next FFT for MDCT
*
**********************************************************************************/
static void MDCT_Second(int *buf0, int num, const int *csptr)
{
	int i;
	int tr1, ti1, tr2, ti2;
	int cosa, sina, cosb, sinb;
	int *buf1;
	int acc0, acc1, acc2;

	buf1 = buf0 + num - 1;
	i = num >> 2;
//	for(i = num >> 2; i != 0; i--)
	do {
		cosa = *csptr++;
		sina = *csptr++;
		cosb = *csptr++;
		sinb = *csptr++;

		tr1 = *(buf0 + 0);
		ti1 = *(buf0 + 1);
		ti2 = *(buf1 + 0);
		tr2 = *(buf1 - 1);
#ifdef LINUX
		asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (cosa), [i2] "r" (tr1));
		asm volatile("smlal %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (sina), [i2] "r" (ti1));
		*buf0++ = acc0;
		asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (sina), [i2] "r" (tr1));
		asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc1) : [i1] "r" (cosa), [i2] "r" (ti1));
		*buf1-- = acc0 - acc1;
		asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (sinb), [i2] "r" (tr2));
		asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc1) : [i1] "r" (cosb), [i2] "r" (ti2));
		*buf0++ = acc0 - acc1;
		asm volatile("smull %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (cosb), [i2] "r" (tr2));
		asm volatile("smlal %[o1], %[o2], %[i1], %[i2]" : [o1] "+r" (acc2), [o2] "+r" (acc0) : [i1] "r" (sinb), [i2] "r" (ti2));
#else
		__asm {
			SMULL	acc2, acc0, cosa, tr1
//			MOV		acc2, #0
			SMLAL	acc2, acc0, sina, ti1
		}
		*buf0++ = acc0;
		__asm {
			SMULL	acc2, acc0, sina, tr1
			SMULL	acc2, acc1, cosa, ti1
		}
		*buf1-- = acc0 - acc1;

		__asm {
			SMULL	acc2, acc0, sinb, tr2
			SMULL	acc2, acc1, cosb, ti2
		}
		*buf0++ = acc0 - acc1;
		__asm {
			SMULL	acc2, acc0, cosb, tr2
//			MOV		acc2, #0
			SMLAL	acc2, acc0, sinb, ti2
		}
#endif
		*buf1-- = acc0;

//		*buf0++ = MULHIGH(cosa, tr1) + MULHIGH(sina, ti1);
//		*buf1-- = MULHIGH(sina, tr1) - MULHIGH(cosa, ti1);
//		*buf0++ = MULHIGH(sinb, tr2) - MULHIGH(cosb, ti2);
//		*buf1-- = MULHIGH(cosb, tr2) + MULHIGH(sinb, ti2);
	} while (--i!=0);
}
#endif


/**********************************************************************************
*
* function name: MDCT_BlkLong
* description:  the long block mdct, include long_start block, end_long block
*
**********************************************************************************/
void MDCT_BlkLong(int *buf)
{
	//MDCT_One(buf, 1024, cossintab + 128);

    MDCT_One(buf, 1024, cossintab_1024);

	//ShuffleSwap(buf, 512, bitrevTab + 17);
	ShuffleSwap(buf, 512, bitrevTab_129);
	Radix8FFT(buf, 512 >> 3);
	Radix4FFT(buf, 512 >> 3, 8, (int *)twidTab512);

	//MDCT_Second(buf, 1024, cossintab + 128);
	MDCT_Second(buf, 1024, cossintab_1024);
}


/**********************************************************************************
*
* function name: MDCT_BlkShort
* description:  the short block mdct
*
**********************************************************************************/
void MDCT_BlkShort(int *buf)
{
	//MDCT_One(buf, 128, cossintab);
	MDCT_One(buf, 128, cossintab_128);

	//ShuffleSwap(buf, 64, bitrevTab);
	ShuffleSwap(buf, 64, bitrevTab_17);
	Radix4_1(buf, 64 >> 2);
	Radix4FFT(buf, 64 >> 2, 4, (int *)twidTab64);

	//MDCT_Second(buf, 128, cossintab);
	MDCT_Second(buf, 128, cossintab_128);
}


/*****************************************************************************
*
* function name: MDCT_ShifDelayBuf
* description:    the mdct delay buffer has a size of 1600,
*  so the calculation of LONG,STOP must be  spilt in two
*  passes with 1024 samples and a mid shift,
*  the SHORT transforms can be completed in the delay buffer,
*  and afterwards a shift
*
**********************************************************************************/
static void MDCT_ShifDelayBuf(int16_t *mdctDelayBuffer, /*! start of mdct delay buffer */
								 int16_t *timeSignal,      /*! pointer to new time signal samples, interleaved */
								 int16_t chIncrement       /*! number of channels */
								 )
{
	int32_t i;
	int16_t *srBuf = mdctDelayBuffer;
	int16_t *dsBuf = mdctDelayBuffer+FRAME_LEN_LONG;

	for(i = 0; i < BLOCK_SWITCHING_OFFSET-FRAME_LEN_LONG; i+= 8)
	{
		*srBuf++ = *dsBuf++;	 *srBuf++ = *dsBuf++;
		*srBuf++ = *dsBuf++;	 *srBuf++ = *dsBuf++;
		*srBuf++ = *dsBuf++;	 *srBuf++ = *dsBuf++;
		*srBuf++ = *dsBuf++;	 *srBuf++ = *dsBuf++;
	}

	srBuf = mdctDelayBuffer + BLOCK_SWITCHING_OFFSET-FRAME_LEN_LONG;
	dsBuf = timeSignal;

	for(i=0; i<FRAME_LEN_LONG; i+=8)
	{
		*srBuf++ = *dsBuf; dsBuf += chIncrement;
		*srBuf++ = *dsBuf; dsBuf += chIncrement;
		*srBuf++ = *dsBuf; dsBuf += chIncrement;
		*srBuf++ = *dsBuf; dsBuf += chIncrement;
		*srBuf++ = *dsBuf; dsBuf += chIncrement;
		*srBuf++ = *dsBuf; dsBuf += chIncrement;
		*srBuf++ = *dsBuf; dsBuf += chIncrement;
		*srBuf++ = *dsBuf; dsBuf += chIncrement;
	}
}


/*****************************************************************************
*
* function name: MDCT_CalcScalefactor
* description:  Calculate max possible scale factor for input vector of shorts
* returns:      Maximum scale factor
*
**********************************************************************************/
static int16_t MDCT_CalcScalefactor(const int16_t *vector, /*!< Pointer to input vector */
												int16_t len,           /*!< Length of input vector */
												int16_t stride)        /*!< Stride of input vector */
{
	int32_t maxVal = 0;
	int32_t absVal;
	int32_t i, tmp;
	int16_t *vector_p;

	vector_p = (int16_t *)vector;
	i = len;
//	for(i=0; i<len; i++){
	do {
		tmp = *vector_p;
		vector_p += stride;
		absVal = my_abs(tmp);
		maxVal |= absVal;
	} while (--i!=0);

	return( maxVal ? norm_s(maxVal) : 15);
}


/*****************************************************************************
*
* function name: MDCT_Trans
* description:  Calculate transform filter for input vector of shorts
* returns:      TRUE if success
*
**********************************************************************************/
void MDCT_Trans(int16_t *mdctDelayBuffer,
                    int16_t *timeSignal,
                    int16_t chIncrement,
                    int32_t *realOut,
                    int16_t *mdctScale,
                    int16_t blockType
                    )
{
	int32_t i,w;
	int32_t timeSignalSample;
	int32_t ws1,ws2;
	int16_t *dctIn0, *dctIn1;
	int32_t *outData0, *outData1;
	int32_t *winPtr;

	int32_t delayBufferSf,timeSignalSf,minSf;
	int16_t *mdctDelayBuffer_p1;

	switch(blockType){


	case LONG_WINDOW:
		/*
		we access BLOCK_SWITCHING_OFFSET (1600 ) delay buffer samples + 448 new timeSignal samples
		and get the biggest scale factor for next calculate more precise
		*/
		delayBufferSf = MDCT_CalcScalefactor(mdctDelayBuffer,BLOCK_SWITCHING_OFFSET,1);
		timeSignalSf  = MDCT_CalcScalefactor(timeSignal,2*FRAME_LEN_LONG-BLOCK_SWITCHING_OFFSET,chIncrement);
		minSf = min(delayBufferSf,timeSignalSf);
		minSf = min(minSf,14);

		dctIn0 = mdctDelayBuffer;
		dctIn1 = mdctDelayBuffer + FRAME_LEN_LONG - 1;
		outData0 = realOut + FRAME_LEN_LONG/2;

		/* add windows and pre add for mdct to last buffer*/
		winPtr = (int *)LongWindowKBD;
//		for(i=0;i<FRAME_LEN_LONG/2;i++){
		i = FRAME_LEN_LONG/2;
		do {
			timeSignalSample = (*dctIn0++) << minSf;
			ws1 = timeSignalSample * (*winPtr >> 16);
			timeSignalSample = (*dctIn1--) << minSf;
			ws2 = timeSignalSample * (*winPtr & 0xffff);
			winPtr ++;
			/* shift 2 to avoid overflow next */
			*outData0++ = (ws1 >> 2) - (ws2 >> 2);
		} while (--i!=0);

		MDCT_ShifDelayBuf(mdctDelayBuffer,timeSignal,chIncrement);

		/* add windows and pre add for mdct to new buffer*/
		dctIn0 = mdctDelayBuffer;
		dctIn1 = mdctDelayBuffer + FRAME_LEN_LONG - 1;
		outData0 = realOut + FRAME_LEN_LONG/2 - 1;
		winPtr = (int *)LongWindowKBD;
		i = FRAME_LEN_LONG/2;
//		for(i=0;i<FRAME_LEN_LONG/2;i++){
		do {
			timeSignalSample = (*dctIn0++) << minSf;
			ws1 = timeSignalSample * (*winPtr & 0xffff);
			timeSignalSample = (*dctIn1--) << minSf;
			ws2 = timeSignalSample * (*winPtr >> 16);
			winPtr++;
			/* shift 2 to avoid overflow next */
			*outData0-- = -((ws1 >> 2) + (ws2 >> 2));
		} while (--i!=0);

		MDCT_BlkLong(realOut);
		/* update scale factor */
		minSf = 14 - minSf;
		*mdctScale=minSf;
		break;

	case START_WINDOW:
		/*
		we access BLOCK_SWITCHING_OFFSET (1600 ) delay buffer samples + no timeSignal samples
		and get the biggest scale factor for next calculate more precise
		*/
		minSf = MDCT_CalcScalefactor(mdctDelayBuffer,BLOCK_SWITCHING_OFFSET,1);
		minSf = min(minSf,14);

		dctIn0 = mdctDelayBuffer;
		dctIn1 = mdctDelayBuffer + FRAME_LEN_LONG - 1;
		outData0 = realOut + FRAME_LEN_LONG/2;
		winPtr = (int *)LongWindowKBD;

		/* add windows and pre add for mdct to last buffer*/
//		for(i=0;i<FRAME_LEN_LONG/2;i++){
		i = FRAME_LEN_LONG/2;
		do {
			timeSignalSample = (*dctIn0++) << minSf;
			ws1 = timeSignalSample * (*winPtr >> 16);
			timeSignalSample = (*dctIn1--) << minSf;
			ws2 = timeSignalSample * (*winPtr & 0xffff);
			winPtr ++;
			*outData0++ = (ws1 >> 2) - (ws2 >> 2);  /* shift 2 to avoid overflow next */
		} while (--i!=0);

		MDCT_ShifDelayBuf(mdctDelayBuffer,timeSignal,chIncrement);

		outData0 = realOut + FRAME_LEN_LONG/2 - 1;
		for(i=0;i<LS_TRANS;i++){
			*outData0-- = -mdctDelayBuffer[i] << (15 - 2 + minSf);
		}

		/* add windows and pre add for mdct to new buffer*/
		dctIn0 = mdctDelayBuffer + LS_TRANS;
		dctIn1 = mdctDelayBuffer + FRAME_LEN_LONG - 1 - LS_TRANS;
		outData0 = realOut + FRAME_LEN_LONG/2 - 1 -LS_TRANS;
		winPtr = (int *)ShortWindowSine;
//		for(i=0;i<FRAME_LEN_SHORT/2;i++){
		i = FRAME_LEN_SHORT/2;
		do {
			timeSignalSample= (*dctIn0++) << minSf;
			ws1 = timeSignalSample * (*winPtr & 0xffff);
			timeSignalSample= (*dctIn1--) << minSf;
			ws2 = timeSignalSample * (*winPtr >> 16);
			winPtr++;
			*outData0-- =  -((ws1 >> 2) + (ws2 >> 2));  /* shift 2 to avoid overflow next */
		} while (--i!=0);

		MDCT_BlkLong(realOut);
		/* update scale factor */
		minSf = 14 - minSf;
		*mdctScale= minSf;
		break;

	case STOP_WINDOW:
		/*
		we access BLOCK_SWITCHING_OFFSET-LS_TRANS (1600-448 ) delay buffer samples + 448 new timeSignal samples
		and get the biggest scale factor for next calculate more precise
		*/
		delayBufferSf = MDCT_CalcScalefactor(mdctDelayBuffer+LS_TRANS,BLOCK_SWITCHING_OFFSET-LS_TRANS,1);
		timeSignalSf  = MDCT_CalcScalefactor(timeSignal,2*FRAME_LEN_LONG-BLOCK_SWITCHING_OFFSET,chIncrement);
		minSf = min(delayBufferSf,timeSignalSf);
		minSf = min(minSf,13);

		outData0 = realOut + FRAME_LEN_LONG/2;
		dctIn1 = mdctDelayBuffer + FRAME_LEN_LONG - 1;
//		for(i=0;i<LS_TRANS;i++){
		i = LS_TRANS;
		do {
			*outData0++ = -(*dctIn1--) << (15 - 2 + minSf);
		} while (--i!=0);

		/* add windows and pre add for mdct to last buffer*/
		dctIn0 = mdctDelayBuffer + LS_TRANS;
		dctIn1 = mdctDelayBuffer + FRAME_LEN_LONG - 1 - LS_TRANS;
		outData0 = realOut + FRAME_LEN_LONG/2 + LS_TRANS;
		winPtr = (int *)ShortWindowSine;
//		for(i=0;i<FRAME_LEN_SHORT/2;i++){
		i = FRAME_LEN_SHORT/2;
		do {
			timeSignalSample = (*dctIn0++) << minSf;
			ws1 = timeSignalSample * (*winPtr >> 16);
			timeSignalSample= (*dctIn1--) << minSf;
			ws2 = timeSignalSample * (*winPtr & 0xffff);
			winPtr++;
			*outData0++ = (ws1 >> 2) - (ws2 >> 2);  /* shift 2 to avoid overflow next */
		} while (--i!=0);

		MDCT_ShifDelayBuf(mdctDelayBuffer,timeSignal,chIncrement);

		/* add windows and pre add for mdct to new buffer*/
		dctIn0 = mdctDelayBuffer;
		dctIn1 = mdctDelayBuffer + FRAME_LEN_LONG - 1;
		outData0 = realOut + FRAME_LEN_LONG/2 - 1;
		winPtr = (int *)LongWindowKBD;
//		for(i=0;i<FRAME_LEN_LONG/2;i++){
		i = FRAME_LEN_LONG/2;
		do {
			timeSignalSample= (*dctIn0++) << minSf;
			ws1 = timeSignalSample *(*winPtr & 0xffff);
			timeSignalSample= (*dctIn1--) << minSf;
			ws2 = timeSignalSample * (*winPtr >> 16);
			*outData0-- =  -((ws1 >> 2) + (ws2 >> 2));  /* shift 2 to avoid overflow next */
			winPtr++;
		} while (--i!=0);

		MDCT_BlkLong(realOut);
		minSf = 14 - minSf;
		*mdctScale= minSf; /* update scale factor */
		break;

	case SHORT_WINDOW:
		/*
		we access BLOCK_SWITCHING_OFFSET (1600 ) delay buffer samples + no new timeSignal samples
		and get the biggest scale factor for next calculate more precise
		*/
		minSf = MDCT_CalcScalefactor(mdctDelayBuffer+TRANSFORM_OFFSET_SHORT,9*FRAME_LEN_SHORT,1);
		minSf = min(minSf,10);

		mdctDelayBuffer_p1 = mdctDelayBuffer + TRANSFORM_OFFSET_SHORT - FRAME_LEN_SHORT;
//		for(w=0;w<TRANS_FAC;w++){
		w = TRANS_FAC;
		do {
			mdctDelayBuffer_p1 += FRAME_LEN_SHORT;
			dctIn0 = mdctDelayBuffer_p1;
			dctIn1 = mdctDelayBuffer_p1 + FRAME_LEN_SHORT-1;
//			dctIn0 = mdctDelayBuffer+w*FRAME_LEN_SHORT+TRANSFORM_OFFSET_SHORT;
//			dctIn1 = mdctDelayBuffer+w*FRAME_LEN_SHORT+TRANSFORM_OFFSET_SHORT + FRAME_LEN_SHORT-1;
			outData0 = realOut + FRAME_LEN_SHORT/2;
			outData1 = realOut + FRAME_LEN_SHORT/2 - 1;

			winPtr = (int *)ShortWindowSine;
//			for(i=0;i<FRAME_LEN_SHORT/2;i++){
			i = FRAME_LEN_SHORT/2;
			do {
				timeSignalSample= *dctIn0 << minSf;
				ws1 = timeSignalSample * (*winPtr >> 16);
				timeSignalSample= *dctIn1 << minSf;
				ws2 = timeSignalSample * (*winPtr & 0xffff);
				*outData0++ = (ws1 >> 2) - (ws2 >> 2);  /* shift 2 to avoid overflow next */

				timeSignalSample= *(dctIn0 + FRAME_LEN_SHORT) << minSf;
				ws1 = timeSignalSample * (*winPtr & 0xffff);
				timeSignalSample= *(dctIn1 + FRAME_LEN_SHORT) << minSf;
				ws2 = timeSignalSample * (*winPtr >> 16);
				*outData1-- =  -((ws1 >> 2) + (ws2 >> 2));  /* shift 2 to avoid overflow next */

				winPtr++;
				dctIn0++;
				dctIn1--;
			} while (--i!=0);

			MDCT_BlkShort(realOut);
			realOut += FRAME_LEN_SHORT;
		} while (--w!=0);

		minSf = 11 - minSf;
		*mdctScale = minSf; /* update scale factor */

		MDCT_ShifDelayBuf(mdctDelayBuffer,timeSignal,chIncrement);
		break;
  }
}
