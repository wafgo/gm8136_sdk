...
**
** $Id: lt_predict.c,v 1.1 2013/09/04 06:26:55 easonli Exp $
**/
...
void lt_prediction(ic_stream *ics, ltp_info *ltp, real_t *spec,
                   short *lt_pred_stat, fb_info *fb, char win_shape,
                   char win_shape_prev, char sr_index,
        ...
        if (ltp->data_present)
        {
            ...
            for (sfb = 0; sfb < ltp->last_band; sfb++)
            {
                if (ltp->long_used[sfb])
                {
                    ...
                    unsigned short high = ics->swb_offset[sfb+1];
...
#ifdef FIXED_POINT
static INLINE short real_to_int16(real_t sig_in)
{
    if (sig_in >= 0)
    {
...
void lt_update_state(short *lt_pred_stat, real_t *time, real_t *overlap,
                     unsigned short frame_len, char object_type)
{
    unsigned short i;
