...
**
** $Id: mp4atom.c,v 1.1 2013/09/04 06:26:55 easonli Exp $
**/
...
static unsigned char mp4ff_atom_name_to_type(const signed char a, const signed char b,
                                       const signed char c, const signed char d)
{
    if (a == 'm')
    {
...
static int mp4ff_read_stsz(mp4ff_t *f)
{
    ...
    if (f->track[f->total_tracks - 1]->stsz_sample_size == 0)
    {
        ...
        f->track[f->total_tracks - 1]->stsz_table =
            (int*)malloc(f->track[f->total_tracks - 1]->stsz_sample_count*sizeof(int));
...
static int mp4ff_read_esds(mp4ff_t *f)
{
    ...
    if (f->track[f->total_tracks - 1]->decoderConfig)
    {
        mp4ff_read_data(f, (signed char *)f->track[f->total_tracks - 1]->decoderConfig, f->track[f->total_tracks - 1]->decoderConfigLen);
    } else {
...
static int mp4ff_read_stsd(mp4ff_t *f)
{
    ...
    {
        unsigned long long skip = mp4ff_position(f);
        unsigned long long size;
        unsigned char atom_type = 0;
        size = mp4ff_atom_read_header(f, &atom_type, &header_size);
        skip += size;
...
static int mp4ff_read_stsc(mp4ff_t *f)
{
    ...
    f->track[f->total_tracks - 1]->stsc_first_chunk =
        (int*)malloc(f->track[f->total_tracks - 1]->stsc_entry_count*sizeof(int));
...
static int mp4ff_read_stco(mp4ff_t *f)
{
    ...
    f->track[f->total_tracks - 1]->stco_chunk_offset =
        (int*)malloc(f->track[f->total_tracks - 1]->stco_entry_count*sizeof(int));
...
static int mp4ff_read_ctts(mp4ff_t *f)
{
    int i;
    ...
    p_track->ctts_sample_count = (int*)malloc(p_track->ctts_entry_count * sizeof(int));
    p_track->ctts_sample_offset = (int*)malloc(p_track->ctts_entry_count * sizeof(int));
...
static int mp4ff_read_stts(mp4ff_t *f)
{
    ...
    p_track->stts_sample_count = (int*)malloc(p_track->stts_entry_count * sizeof(int));
    p_track->stts_sample_delta = (int*)malloc(p_track->stts_entry_count * sizeof(int));
...
static int mp4ff_read_mvhd(mp4ff_t *f)
{
    int i;
...
#if 0
static int mp4ff_read_tkhd(mp4ff_t *f)
{
...
static int mp4ff_read_mdhd(mp4ff_t *f)
{
	...
	else //version == 0
	{
		...
		f->track[f->total_tracks - 1]->duration = (temp == (unsigned int)(-1)) ? (unsigned long long)(-1) : (unsigned long long)(temp);
	}
	mp4ff_read_int16(f);
	mp4ff_read_int16(f);
	return 1;
...
{
	unsigned long long dest_position = mp4ff_position(f)+size-8;
