...
**
** $Id: pns.c,v 1.1 2013/09/04 06:26:55 easonli Exp $
**/
...
*/
static INLINE void gen_rand_vector(real_t *spec, short scale_factor, unsigned short size,
                                   char sub)
{
#ifndef FIXED_POINT
    unsigned short i;
    ...
    for (i = 0; i < size; i++)
    {
        real_t tmp = scale*(real_t)(int)random_int();
        spec[i] = tmp;
        energy += tmp*tmp;
    }
                ...
                if (is_noise(ics_left, g, sfb))
                {
                    ...
                    */
                    ...
                    gen_rand_vector(&spec_left[(group*nshort)+offs],
                        ics_left->scale_factors[g*51+sfb], size, sub);
                }
                ...
                if (channel_pair)
                {
                    if (is_noise(ics_right, g, sfb))
                    {
                        if (((ics_left->ms_mask_present == 1) &&
                            (ics_left->ms_used[g*MAX_SFB+sfb])) ||
                            (ics_left->ms_mask_present == 2))
                        {
                            ...
                            gen_rand_vector(&spec_right[(group*nshort)+offs],
                                ics_right->scale_factors[g*51+sfb], size, sub);
                        }
                    }
                }
            } /* sfb */
