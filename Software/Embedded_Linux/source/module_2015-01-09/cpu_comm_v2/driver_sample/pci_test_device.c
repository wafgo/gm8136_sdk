...
int test_working_thread_device_pack_rx0(void *private)
{
    ...
    switch(idx) {
      case 0:
        cpu_comm_pack_open(CHAN_PCI0_0_USR0, "Receiver0");
        rcv_msg.target = CHAN_PCI0_0_USR0;
        break;
      case 1:
        cpu_comm_pack_open(CHAN_PCI0_0_USR1, "Receiver1");
        rcv_msg.target = CHAN_PCI0_0_USR1;
        break;
      case 2:
        cpu_comm_pack_open(CHAN_PCI0_0_USR2, "Receiver2");
        rcv_msg.target = CHAN_PCI0_0_USR2;
        break;
      default:
        panic("error! \n");
        break;
    ...
    printk("PCI Slave testing with TEST_SIZE = %d \n", (u32)TEST_SIZE);
    ...
    for (loop = 0; loop < TOTAL_LOOP; loop ++) {
        ...
        if (1) {
            ...
            if (loop != ptr[1]) {
                printk("idx%d, loop value fail! Expected value is %d but %d!\n", idx, loop, ptr[1]);
                msleep(900);
            }

            if (TOTAL_LOOP != ptr[2]) {
                printk("idx%d, total loop value fail! Expected value is %d but %d!\n", idx, TOTAL_LOOP, ptr[2]);
                msleep(900);
            }
        }
        ...
        for (i = 1; i < CPU_COMM_NUM_IN_PACK; i ++) {
            ...
            for (j = 0; j < (rcv_msg.length[i] >> 2); j ++) {
                if (ptr[j] != start ++) {
                    printk("idx%d, data verify fail! Expected value = %d, but get %d, virtual = 0x%x, ofs = %d \n", 
        ...
        if ((loop % 500) == 0)
            printk("idx: %d, loop%d, start = 0x%x \n", idx, loop, start);
    } /* loop */
    ...
    switch (idx) {
      case 0:
        cpu_comm_pack_close(CHAN_PCI0_0_USR0);
        break;
      case 1:
        cpu_comm_pack_close(CHAN_PCI0_0_USR1);
        break;
      case 2:
        cpu_comm_pack_close(CHAN_PCI0_0_USR2);
        break;
      default:
        panic("error! \n");
        break;
...
int test_working_thread_device_rx(void *private)
{
    ...
    /* open a new channel to transmit/receive the data */
    cpu_comm_open(CHAN_PCI0_0_USR0, "Slave");
    ...
    snd_msg.target = CHAN_PCI0_0_USR0;
    ...
    cpu_comm_close(CHAN_PCI0_0_USR0);
    kfree(rbuf);
