...
/* device number */
static int dev_num = TW2964_DEV_MAX;
...
/* device i2c address */
static ushort iaddr[TW2964_DEV_MAX] = {0x50, 0x52, 0x54, 0x56};
module_param_array(iaddr, ushort, NULL, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(iaddr, "Device I2C Address");

/* device video mode */
static int vmode[TW2964_DEV_MAX] = {[0 ... (TW2964_DEV_MAX - 1)] = TW2964_VMODE_NTSC_720H_4CH};
...
/* clock frequency */
static int clk_freq = TW2964_CLKIN;   ///< 27MHz
...
/* audio sample rate */
static int sample_rate = TW2964_AUDIO_SAMPLERATE_8K;
module_param(sample_rate, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(sample_rate, "audio sample rate: 0:8k  1:16k  2:32k 3:44.1k 4:48k");

/* audio sample size */
static int sample_size = TW2964_AUDIO_BITS_16B;
...
static struct tw2964_dev_t    tw2964_dev[TW2964_DEV_MAX];
...
static struct i2c_client  *tw2964_i2c_client = NULL;
static struct task_struct *tw2964_wd         = NULL;
static u32 TW2964_VCH_MAP[TW2964_DEV_MAX*TW2964_DEV_CH_MAX];

static int tw2964_i2c_probe(struct i2c_client *client, const struct i2c_device_id *did)
{
    tw2964_i2c_client = client;
    return 0;
}

static int tw2964_i2c_remove(struct i2c_client *client)
{
    return 0;
}

static const struct i2c_device_id tw2964_i2c_id[] = {
    { "tw2964_i2c", 0 },
    { }
};

static struct i2c_driver tw2964_i2c_driver = {
    .driver = {
        .name  = "TW2964_I2C",
        .owner = THIS_MODULE,
    },
    .probe    = tw2964_i2c_probe,
    .remove   = tw2964_i2c_remove,
    .id_table = tw2964_i2c_id
};

static int tw2964_i2c_register(void)
{
    int ret;
    ...
    /* add i2c driver */
    ret = i2c_add_driver(&tw2964_i2c_driver);
    if(ret < 0) {
        printk("TW2964 can't add i2c driver!\n");
        return ret;
    }

    memset(&info, 0, sizeof(struct i2c_board_info));
    info.addr = iaddr[0]>>1;
    strlcpy(info.type, "tw2964_i2c", I2C_NAME_SIZE);

    adapter = i2c_get_adapter(ibus);   ///< I2C BUS#
    if (!adapter) {
        printk("TW2964 can't get i2c adapter\n");
        goto err_driver;
    }
    ...
    if(!client) {
        printk("TW2964 can't add i2c device!\n");
        goto err_driver;
    }
...
err_driver:
   i2c_del_driver(&tw2964_i2c_driver);
   return -ENODEV;
}

static void tw2964_i2c_unregister(void)
{
    i2c_unregister_device(tw2964_i2c_client);
    i2c_del_driver(&tw2964_i2c_driver);
    tw2964_i2c_client = NULL;
}

static int tw2964_create_ch_map(int map_id)
{
    int i;
    ...
    switch(map_id) {
        case 0:     ///< for GM8210/GM8287 Socket Board
            for(i=0; i<dev_num; i++) {
                TW2964_VCH_MAP[(i*TW2964_DEV_VPORT_MAX)+0] = CH_IDENTIFY(i, 0, TW2964_DEV_VPORT_VD2);
                TW2964_VCH_MAP[(i*TW2964_DEV_VPORT_MAX)+1] = CH_IDENTIFY(i, 1, TW2964_DEV_VPORT_VD2);
                TW2964_VCH_MAP[(i*TW2964_DEV_VPORT_MAX)+2] = CH_IDENTIFY(i, 2, TW2964_DEV_VPORT_VD2);
                TW2964_VCH_MAP[(i*TW2964_DEV_VPORT_MAX)+3] = CH_IDENTIFY(i, 3, TW2964_DEV_VPORT_VD2);
            }
            break;
        case 1:     ///< General, 1CH Mode
            for(i=0; i<dev_num; i++) {
                TW2964_VCH_MAP[(i*TW2964_DEV_VPORT_MAX)+0] = CH_IDENTIFY(i, 0, TW2964_DEV_VPORT_VD1);
                TW2964_VCH_MAP[(i*TW2964_DEV_VPORT_MAX)+1] = CH_IDENTIFY(i, 1, TW2964_DEV_VPORT_VD2);
                TW2964_VCH_MAP[(i*TW2964_DEV_VPORT_MAX)+2] = CH_IDENTIFY(i, 2, TW2964_DEV_VPORT_VD3);
                TW2964_VCH_MAP[(i*TW2964_DEV_VPORT_MAX)+3] = CH_IDENTIFY(i, 3, TW2964_DEV_VPORT_VD4);
            }
            break;
        case 2:     ///< General, 2CH Mode
            for(i=0; i<dev_num; i++) {
                TW2964_VCH_MAP[(i*TW2964_DEV_VPORT_MAX)+0] = CH_IDENTIFY(i, 0, TW2964_DEV_VPORT_VD1);
                TW2964_VCH_MAP[(i*TW2964_DEV_VPORT_MAX)+1] = CH_IDENTIFY(i, 1, TW2964_DEV_VPORT_VD1);
                TW2964_VCH_MAP[(i*TW2964_DEV_VPORT_MAX)+2] = CH_IDENTIFY(i, 2, TW2964_DEV_VPORT_VD2);
                TW2964_VCH_MAP[(i*TW2964_DEV_VPORT_MAX)+3] = CH_IDENTIFY(i, 3, TW2964_DEV_VPORT_VD2);
            }
            break;
        case 3:     ///< General, 4CH Mode
        default:
            for(i=0; i<dev_num; i++) {
                TW2964_VCH_MAP[(i*TW2964_DEV_VPORT_MAX)+0] = CH_IDENTIFY(i, 0, TW2964_DEV_VPORT_VD1);
                TW2964_VCH_MAP[(i*TW2964_DEV_VPORT_MAX)+1] = CH_IDENTIFY(i, 1, TW2964_DEV_VPORT_VD1);
                TW2964_VCH_MAP[(i*TW2964_DEV_VPORT_MAX)+2] = CH_IDENTIFY(i, 2, TW2964_DEV_VPORT_VD1);
                TW2964_VCH_MAP[(i*TW2964_DEV_VPORT_MAX)+3] = CH_IDENTIFY(i, 3, TW2964_DEV_VPORT_VD1);
            }
            break;
    }
...
int tw2964_get_vch_id(int id, TW2964_DEV_VPORT_T vport, int vport_seq)
{
    ...
    for(i=0; i<(dev_num*TW2964_DEV_CH_MAX); i++) {
        if((CH_IDENTIFY_ID(TW2964_VCH_MAP[i]) == id) &&
           (CH_IDENTIFY_VPORT(TW2964_VCH_MAP[i]) == vport) &&
           ((CH_IDENTIFY_VIN(TW2964_VCH_MAP[i])%vport_chnum) == vport_seq)) {
            return i;
        }
    }
...
int tw2964_vin_to_ch(int id, int vin_idx)
{
    ...
    for(i=0; i<(dev_num*TW2964_DEV_CH_MAX); i++) {
        if((CH_IDENTIFY_ID(TW2964_VCH_MAP[i]) == id) && (CH_IDENTIFY_VIN(TW2964_VCH_MAP[i]) == vin_idx))
...
int tw2964_ch_to_vin(int id, int ch_idx)
{
    ...
    for(i=0; i<(dev_num*TW2964_DEV_CH_MAX); i++) {
        if((CH_IDENTIFY_ID(TW2964_VCH_MAP[i]) == id) && ((i%TW2964_DEV_CH_MAX) == ch_idx))
            return CH_IDENTIFY_VIN(TW2964_VCH_MAP[i]);
    }
...
int tw2964_notify_vlos_register(int id, int (*nt_func)(int, int, int))
{
    ...
    down(&tw2964_dev[id].lock);

    tw2964_dev[id].vlos_notify = nt_func;

    up(&tw2964_dev[id].lock);

    return 0;
}
EXPORT_SYMBOL(tw2964_notify_vlos_register);
...
void tw2964_notify_vlos_deregister(int id)
{
    ...
    tw2964_dev[id].vlos_notify = NULL;

    up(&tw2964_dev[id].lock);
}
EXPORT_SYMBOL(tw2964_notify_vlos_deregister);
...
int tw2964_notify_vfmt_register(int id, int (*nt_func)(int, int))
{
    ...
    tw2964_dev[id].vfmt_notify = nt_func;

    up(&tw2964_dev[id].lock);

    return 0;
}
EXPORT_SYMBOL(tw2964_notify_vfmt_register);
...
void tw2964_notify_vfmt_deregister(int id)
{
    ...
    tw2964_dev[id].vfmt_notify = NULL;

    up(&tw2964_dev[id].lock);
}
EXPORT_SYMBOL(tw2964_notify_vfmt_deregister);

/* return value, 0: success, others: failed */
static int tw2964_sound_switch(int ssp_idx, int chan_num, bool is_on)
{
...
int tw2964_i2c_write(u8 id, u8 reg, u8 data)
{
    ...
    if(!tw2964_i2c_client) {
        printk("TW2964 i2c_client not register!!\n");
        return -1;
    }

    adapter = to_i2c_adapter(tw2964_i2c_client->dev.parent);
    ...
    if(i2c_transfer(adapter, &msgs, 1) != 1) {
        printk("TW2964#%d i2c write failed!!\n", id);
        return -1;
    }
...
u8 tw2964_i2c_read(u8 id, u8 reg)
{
    ...
    if(i2c_transfer(adapter, msgs, 2) != 2)
        printk("TW2964#%d i2c read failed!!\n", id);
...
EXPORT_SYMBOL(tw2964_get_device_num);
...
static int tw2964_proc_status_show(struct seq_file *sfile, void *v)
{
    ...
    for(i=0; i<TW2964_DEV_CH_MAX; i++) {
        for(j=0; j<(dev_num*TW2964_DEV_CH_MAX); j++) {
            if((CH_IDENTIFY_ID(TW2964_VCH_MAP[j]) == pdev->index) && (CH_IDENTIFY_VIN(TW2964_VCH_MAP[j]) == i)) {
...
static int tw2964_proc_init(int id)
{
    int ret = 0;
...
static int tw2964_miscdev_init(int id)
{
    int ret;
...
static void tw2964_hw_reset(void)
{
    if(!rstb_used || !init)
        return;
...
static int tw2964_device_init(int id)
{
    int ret = 0;
...
static int tw2964_watchdog_thread(void *data)
{
    int i, vin, novid, vmode;
    struct tw2964_dev_t *pdev;

    do {
            /* check tw2964 video status */
            for(i=0; i<dev_num; i++) {
                pdev = &tw2964_dev[i];
                ...
                /* video signal check */
                if(pdev->vlos_notify) {
                    for(vin=0; vin<TW2964_DEV_CH_MAX; vin++) {
                        novid = tw2964_status_get_novid(pdev->index, vin);
                        if(novid)
                ...
                if(pdev->vfmt_notify) {
                    vmode = tw2964_video_get_mode(pdev->index);
                    if((vmode >= 0) && (vmode < TW2964_VMODE_MAX))
...
static int __init tw2964_init(void)
{
    int i, ret;
    ...
    /* check device number */
    if(dev_num > TW2964_DEV_MAX) {
        printk("TW2964 dev_num=%d invalid!!(Max=%d)\n", dev_num, TW2964_DEV_MAX);
        return -1;
    }

    /* register i2c client for contol tw2964 */
    ret = tw2964_i2c_register();
    if(ret < 0)
        return -1;
    ...
    ret = tw2964_create_ch_map(ch_map);
    if(ret < 0)
        goto err;
    ...
    if(init && notify) {
        /* init tw2964 watchdog thread for monitor video status */
        tw2964_wd = kthread_create(tw2964_watchdog_thread, NULL, "tw2964_wd");
        if(!IS_ERR(tw2964_wd))
            wake_up_process(tw2964_wd);
        else {
            printk("create tw2964 watchdog thread failed!!\n");
            tw2964_wd = 0;
            ret = -EFAULT;
            goto err;
        }
...
err:
    if(tw2964_wd)
        kthread_stop(tw2964_wd);

    tw2964_i2c_unregister();
...
static void __exit tw2964_exit(void)
{
    int i;

    /* stop tw2964 watchdog thread */
    if(tw2964_wd)
        kthread_stop(tw2964_wd);
...
module_init(tw2964_init);
module_exit(tw2964_exit);

MODULE_DESCRIPTION("Grain Media TW2964 Video Decoders and Audio Codecs Driver");
