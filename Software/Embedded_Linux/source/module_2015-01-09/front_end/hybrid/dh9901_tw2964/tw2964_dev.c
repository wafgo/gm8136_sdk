/*
 * @file tw2964_dev.c
 * Intersil TW2964 4-CH 960H/D1 Video Decoders and Audio Codecs Driver (for hybrid design)
 * Copyright (C) 2014 GM Corp. (http://www.grain-media.com)
 *
 * $Revision: 1.1 $
 * $Date: 2014/08/05 09:32:30 $
 *
 * ChangeLog:
 *  $Log: tw2964_dev.c,v $
...
{
    return dh9901_tw2964_i2c_write(iaddr[id], reg, data);
}
EXPORT_SYMBOL(tw2964_i2c_write);

u8 tw2964_i2c_read(u8 id, u8 reg)
{
    return dh9901_tw2964_i2c_read(iaddr[id], reg);
}
...
int tw2964_get_vch_id(int id, TW2964_DEV_VPORT_T vport, int vport_seq)
{
    ...
    for(i=0; i<(dev_num*TW2964_DEV_CH_MAX); i++) {
        if((CH_IDENTIFY_ID(DH9901_TW2964_VCH_MAP[i]) == id) &&
           (CH_IDENTIFY_VOUT(DH9901_TW2964_VCH_MAP[i]) == vport) &&
           ((CH_IDENTIFY_VIN(DH9901_TW2964_VCH_MAP[i])%vport_chnum) == vport_seq)) {
            return i;
        }
    }
...
int tw2964_vin_to_ch(int id, int vin_idx)
{
    ...
    for(i=0; i<(dev_num*TW2964_DEV_CH_MAX); i++) {
        if((CH_IDENTIFY_ID(DH9901_TW2964_VCH_MAP[i]) == id) && (CH_IDENTIFY_VIN(DH9901_TW2964_VCH_MAP[i]) == vin_idx))
...
int tw2964_ch_to_vin(int id, int ch_idx)
{
    ...
    for(i=0; i<(dev_num*TW2964_DEV_CH_MAX); i++) {
        if((CH_IDENTIFY_ID(DH9901_TW2964_VCH_MAP[i]) == id) && ((i%TW2964_DEV_CH_MAX) == ch_idx))
            return CH_IDENTIFY_VIN(DH9901_TW2964_VCH_MAP[i]);
    }
