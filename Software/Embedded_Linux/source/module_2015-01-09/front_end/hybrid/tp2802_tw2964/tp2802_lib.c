...
static const unsigned char TP2802_COMM[] = {
    ...
    0x4c, 0x32,
    0x4d, 0x0f,
    0x4e, 0x8f, // Force CLKO4 output driver to 20mA, others are 12mA
    0x4f, 0x0f,
    0x50, 0x00,
    0x51, 0x0a,
    0x52, 0x0b,
...
int tp2802_video_get_video_output_format(int id, int ch, struct tp2802_video_fmt_t *vfmt)
{
    ...
    ret = tp2802_i2c_read_ext(id, 0x1c);
    if(ret < 0)
        return -1;
    npxl_value = (ret & 0xFF)<<8;

    ret = tp2802_i2c_read_ext(id, 0x1d);
    if(ret < 0)
        return -1;
    npxl_value |= ret & 0xFF;
...
int tp2802_status_get_video_loss(int id, int ch)
{
    ...
    ret = tp2802_i2c_read_ext(id, 0x01);
    if(ret < 0)
        return ret;
...
int tp2802_status_get_video_input_format(int id, int ch, struct tp2802_video_fmt_t *vfmt)
{
    ...
    ret = tp2802_i2c_read_ext(id, 0x04);
    if(ret < 0)
        return -1;

    status = ret & 0xFF;
    if(status < 0x30) {
        ret = tp2802_i2c_read_ext(id, 0x03);
        if(ret < 0)
            return -1;
