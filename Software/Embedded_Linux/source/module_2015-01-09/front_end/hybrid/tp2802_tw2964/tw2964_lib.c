...
/** 
 * Note: 1. int tw2964_video_get_mode(int id) was updated to 1-CH mode
 *       2. Video input/output mapping was updated to be inversed (0xCD). 
 */
...
static int tw2964_ntsc_960h_1ch(int id)
{
    ...
    ret = tw2964_i2c_write(id, 0xCD, 0x1B); if(ret < 0) goto exit;  ///< VD1:CH3, VD2:CH2, VD3:CH1, VD4:CH0
...
static int tw2964_pal_960h_1ch(int id)
{
    ...
    ret = tw2964_i2c_write(id, 0x95, 0xAB); if(ret < 0) goto exit;  ///< Improve Color Shift Issue
    ret = tw2964_i2c_write(id, 0xCA, 0x00); if(ret < 0) goto exit;  ///< 1-CH BT.656, VD1/VD2/VD3/VD4
    ret = tw2964_i2c_write(id, 0xCD, 0x1B); if(ret < 0) goto exit;  ///< 1ST VD1:CH3, VD2:CH2, VD3:CH1, VD4:CH0
...
int tw2964_video_get_mode(int id)
{
    ...
    if(is_ntsc) {
        if(is_960h)
            ret = TW2964_VMODE_NTSC_960H_1CH;
        else
            ret = TW2964_VMODE_NTSC_720H_1CH;
    }
    else {
        if(is_960h)
            ret = TW2964_VMODE_PAL_960H_1CH;
        else
            ret = TW2964_VMODE_PAL_720H_1CH;
    }
...
/* Only 1CH mode is supported in hybrid design */
    switch(mode) {
        case TW2964_VMODE_NTSC_720H_1CH:
//        case TW2964_VMODE_NTSC_720H_2CH:
//        case TW2964_VMODE_NTSC_720H_4CH:
