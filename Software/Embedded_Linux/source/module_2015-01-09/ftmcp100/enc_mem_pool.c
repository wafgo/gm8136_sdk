...
static struct mem_buffer_info_t mp4e_ref_buffer = {0, 0, 0};
static struct mem_buffer_info_t mp4e_mb_buffer = {0, 0, 0};
static int cur_layout = -1;
static unsigned int max_chn = MP4E_MAX_CHANNEL;
...
{
    enc_dump_ref_pool(NULL);
    enc_dump_chn_pool(NULL);
...
int parse_enc_layout_gmlib_cfg(void)
{
    ...
    set_fs(fs);
    if (0 == l) {
        mcp100_err("%s does not declare buffer layout of encoder\n", GMLIB_FILENAME);
    }
    return l;
}
#endif
...
static int enc_parse_spec_cfg(void)
{
    char line[MAX_LINE_CHAR];
    char filename[0x80];
...
#ifdef USE_GMLIB_CFG
    if ((l = parse_enc_layout_gmlib_cfg()) > 0) {
        return l;
    }
    l = 0;
#endif
...
int enc_dump_ref_pool(char *str)
{
    int res;
    int cnt = 0;
    int len = 0;
    ...
    dmp_ref("Encoder Reference Pool");
...
int enc_dump_chn_pool(char *str)
{
    int i, chn;
...
int enc_dump_pool_num(char *str)
{
    int res;
    int size, mb_size, cnt;
    int total_size = 0;
    int len = 0;
    //int dec_total_size = 0;
    struct ref_item_t *buf;

    dmp_ref("encoder allocate reference buffer 0x%x/0x%x, size %d", 
        mp4e_ref_buffer.addr_va, mp4e_ref_buffer.addr_pa, mp4e_ref_buffer.size);
    if (mp4e_mb_buffer.addr_va) {
        dmp_ref("encoder allocate mb buffer 0x%x/0x%x, size %d", 
            mp4e_mb_buffer.addr_va, mp4e_mb_buffer.addr_pa, mp4e_mb_buffer.size);
    }
    ...
    for (res = 0; res < MAX_RESOLUTION; res++) {
        if (ref_pool[res].buf_num > 0) {
            ...
            total_size += size + mb_size;
            dmp_ref("\t%s: num %d, size %d + %d", res_base_info[res].name, cnt, size, mb_size);
        }
    }
    dmp_ref("\t\tmpeg4 encoder total size %d byte (%d.%dM)", total_size, total_size / 1048576, (total_size%1048576)*1000/1048576);
    return len;
}

int enc_dump_layout_info(char *str)
{
    int l, i, len = 0;
    int str_len = 0;
    char line[0x100];
...
int enc_clean_mem_pool(void)
{
    ...
    if (ref_item)
        kfree(ref_item);
    ref_item = NULL;
    if (mp4e_ref_buffer.addr_va)
        frm_free_buf_ddr((void *)mp4e_ref_buffer.addr_va);
    if (mp4e_mb_buffer.addr_va)
        frm_free_buf_ddr((void *)mp4e_mb_buffer.addr_va);
    return 0;
}

int enc_init_mem_pool(int max_width, int max_height)
{
    int l, i;
    int ddr_idx;
    int largest_layout_idx = 0;
    unsigned int total_ref_size = 0;
    unsigned int total_mb_size = 0;
    unsigned int max_ref_size = 0;
    unsigned int max_mb_size = 0;
    ...
    if (0 == enc_parse_spec_cfg()) {
        mcp100_err("parsing buffer configure error\n");
        goto init_mem_pool_fail;
    }
    //enc_dump_layout_info(NULL);
    ...
    // remove buffer larger than maximal resolution
    ...
    for (l = 0; l < MAX_LAYOUT_NUM; l++) {
        ...
        for (i = 0; i < MAX_RESOLUTION; i++) {
            if (layout_info[l].ref_pool[i].max_chn) {
                ...
                total_ref_size += get_one_buffer_size(res_base_info[i].width, res_base_info[i].height, IDX_REF_BUF) * buf_num;
                total_mb_size += get_one_buffer_size(res_base_info[i].width, res_base_info[i].height, IDX_MB_BUF) * buf_num;
    ...
    // 4. allocate buffer
    if (max_ref_size > 0) {
        if (allocate_pool_buffer_from_frammap(&mp4e_ref_buffer, max_ref_size, ddr_idx) < 0)
            goto init_mem_pool_fail;
    }
    // encoder mb info (MPEG4)
    if (max_mb_size > 0) {
        if (allocate_pool_buffer_from_frammap(&mp4e_mb_buffer, max_mb_size, ddr_idx) < 0)
            goto init_mem_pool_fail;
    }

    // 5. buffer assignment
    if (fill_ref_item(&mp4e_ref_buffer, &mp4e_mb_buffer, layout_info, ref_item) < 0)
        goto init_mem_pool_fail;
 ...
 */
int enc_register_ref_pool(unsigned int chn, int width, int height)
{
    int i, j;
    unsigned long flags;
...
int enc_deregister_ref_pool(unsigned int chn)
{
    unsigned long flags;
    int j;
    if (chn >= max_chn) {
...
*/
int enc_check_reg_buf_suitable(unsigned char *checker)
{
    int i, ret = 0;
...
int enc_mark_suitable_buf(int chn, unsigned char *checker, int width, int height)
{
    int i, ret = 0;
    int suit_res_type;
    if (chn_pool[chn].suitable_res_type < 0)
...
int enc_allocate_pool_buffer2(struct ref_buffer_info_t *buf, unsigned int res_idx, int chn)
{
    struct ref_item_t *ref;
    unsigned long flags;
    ...
    spin_unlock_irqrestore(&mcp100_pool_lock, flags);
//mcp100_err("{chn%d} allocate 0x%x, mbinfo 0x%x\n", chn, buf->ref_phy, buf->mb_phy);
    return 0;       
}

int enc_release_pool_buffer2(struct ref_buffer_info_t *buf, int chn)
{
    struct ref_item_t *ref;
    int res_idx;
    unsigned long flags;
