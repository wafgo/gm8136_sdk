	...
	/*
	 ...
	 Version 3.14: add function to transform planar 422 to packed 422
	 Version 4.0.1: videograph version2
	 Version 4.0.2: handle bitstream buffer cacheable
	 Version 4.0.3: update preprocess error message
	 Version 4.0.4: add semaphore to protect list (work schedule search start job)
	 Version 4.0.5: update dump property value (hex)
	 Version 4.0.6: platform8136 driver
	 Version 4.0.7: wehn trigger job error, check checksum wait event
	 Version 4.0.8: add parameter to set maximal channel id
	 Version 4.0.9: fix bug of set max channel error
	 */
	#define MJPEGD_VER          0x40000900
	#define MJPEGD_VER_MAJOR	((MJPEGD_VER>>28)&0x0F)     // huge change
	#define MJPEGD_VER_MINOR	((MJPEGD_VER>>20)&0xFF)     // interface change
	#define MJPEGD_VER_MINOR2	((MJPEGD_VER>>8)&0x0FFF)    // functional modified or bug fixed
    #define MJPEGD_VER_BRANCH   (MJPEGD_VER&0xFF)           // branch for customer request
