...
static sensor_reg_t sensor_1080P_init_regs[] = {
    ...
    {DELAY_CODE, 500},
    {0x3044, 0x65},     //DCKRST=1
    {0x3044, 0x25},     //DCKRST=0
...
static u32 get_pclk(u32 xclk)
{
    ...
    isp_info("IMX123 Sensor: pclk(%d) XCLK(%d)\n", pclk, xclk);
...
static void calculate_t_row(void)
{
    ...
    if (g_psensor->img_win.width <= 1920 && g_psensor->img_win.height <= 1080)
        pinfo->t_row = (2933 * 10000) / (g_psensor->pclk / 1000);
    else
        pinfo->t_row = (2250 * 10000) / (g_psensor->pclk / 1000);
    isp_info("t_row=%d\n", pinfo->t_row);
}
...
static void adjust_blanking(int fps)
{
    ...
    isp_info("HMAX=0x%x(%d), VMAX=0x%x(%d)\n",
            pinfo->HMax ,pinfo->HMax ,pinfo->VMax,pinfo->VMax );
...
//            mdelay(pInitTable[i].val);
    ...
    if (sensor_xclk == 54000000){
        write_reg(0x316d, 0x0a);
        write_reg(0x3170, 0x61);
        if (width <= 1920 && height<= 1080)
            write_reg(0x3171, 0x16);
        else
            write_reg(0x3171, 0x18);
    }
...
#define WDR_1080P_LONG_EXP_LINE_MIN    80
#define WDR_1080P_LONG_EXP_LINE_MAX    2240
#define WDR_LONG_EXP_LINE_MIN    80
#define WDR_LONG_EXP_LINE_MAX    3184
...
static int set_exp(u32 exp)
{
    ...
    exp_line = MAX(3, (exp * 1000 + pinfo->t_row / 2 ) / pinfo->t_row);
    //isp_info("set_exp %d, line=%d\n", exp, exp_line);
#if 1
    if (wdr){
        if (g_psensor->img_win.width <= 1920 && g_psensor->img_win.height <= 1080)
            exp_line = MIN(WDR_1080P_LONG_EXP_LINE_MAX, MAX(exp_line, WDR_1080P_LONG_EXP_LINE_MIN));               
        else
            exp_line = MIN(WDR_LONG_EXP_LINE_MAX, MAX(exp_line, WDR_LONG_EXP_LINE_MIN));               
        shs2 = pinfo->VMax - exp_line;
        shs1 = MIN(pinfo->VMax -4, pinfo->VMax - exp_line / 16);
        write_reg(0x3021, (shs2 & 0xFF));
        write_reg(0x3022, ((shs2 >> 8) & 0xFF));
        write_reg(0x3023, ((shs2 >> 16) & 0x1));
...
static int set_exp_us(u32 exp)
{
    ...
    exp_line = MAX(3, (exp * 10 + pinfo->t_row / 2 ) / pinfo->t_row);
...
static int set_wdr_en(int enable)
{
    ...
    if (enable){
        ...
        write_reg(0x3013, 0x00);
        write_reg(0x3018, 0x80);//VMAX
        write_reg(0x3019, 0x0C);
//        write_reg(0x301b, 0xEE);//HMAX
//        write_reg(0x301c, 0x02);
//        write_reg(0x301e, 0xDF);//SHS1
//        write_reg(0x301f, 0x0C);
//        write_reg(0x3021, 0x94);//SHS2
//        write_reg(0x3022, 0x0C);
        write_reg(0x3056, 0x09);
        write_reg(0x3057, 0x0C);
        write_reg(0x3058, 0x64);
        write_reg(0x3065, 0x00);
        write_reg(0x3084, 0x1A);
        write_reg(0x3086, 0x10);
        write_reg(0x30CC, 0x21);
        write_reg(0x30CF, 0xF1);
        write_reg(0x30D0, 0x28);
        write_reg(0x30D2, 0x93);
        write_reg(0x30D3, 0x01);
        write_reg(0x3155, 0x01);
        write_reg(0x31F6, 0x58);    
        write_reg(0x3291, 0x03);   
        write_reg(0x3292, 0xED);   
        write_reg(0x3293, 0x30);   
        write_reg(0x3294, 0x10);   
        write_reg(0x32A0, 0xED);   
        write_reg(0x32A1, 0x30);   
        write_reg(0x32A2, 0x10);
...
//        g_psensor->max_exp = 5000; // 0.5 sec
        ...
        write_reg(0x3013, 0x00);
        write_reg(0x3018, 0x40);//VMAX
        write_reg(0x3019, 0x06);
//        write_reg(0x301b, 0xEE);//HMAX
//        write_reg(0x301c, 0x02);
//        write_reg(0x301e, 0xDF);//SHS1
//        write_reg(0x301f, 0x0C);
...
//        write_reg(0x3022, 0x0C);
        ...
        write_reg(0x3057, 0x0C);
        write_reg(0x3058, 0x64);
        write_reg(0x3065, 0x80);
        write_reg(0x3084, 0x00);
        ...
        write_reg(0x3155, 0x00);
        write_reg(0x31F6, 0x58);    
        write_reg(0x3291, 0x00);   
        write_reg(0x3292, 0x15);   
        write_reg(0x3293, 0xb1);   
        write_reg(0x3294, 0x12);   
        write_reg(0x32A0, 0x15);   
        write_reg(0x32A1, 0xb1);   
        write_reg(0x32A2, 0x12);
        g_psensor->fmt = RAW12;
        g_psensor->min_exp = 1;
        g_psensor->max_exp = 5000; // 0.5 sec
    }
...
// external functions
//=============================================================================
static void imx123_deconstruct(void)
{
...
static int imx123_construct(u32 xclk, u16 width, u16 height)
{
...
static int __init imx123_init(void)
{
    ...
    if ((ret = imx123_construct(sensor_xclk, sensor_w, sensor_h)) < 0)
        goto init_err2;
...
MODULE_DESCRIPTION("Sensor IMX123");
MODULE_LICENSE("GPL");
