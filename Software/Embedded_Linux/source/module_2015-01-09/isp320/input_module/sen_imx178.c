...
// module parameters
//=============================================================================
#define DEFAULT_IMAGE_WIDTH     3072
#define DEFAULT_IMAGE_HEIGHT    2048
...
#define SENSOR_MAX_WIDTH    3072
#define SENSOR_MAX_HEIGHT   2048
...
static sensor_reg_t sensor_1080p_init_regs[] = {
    ...
    {0x33B4, 0xFE},
    {0x33B9, 0x00},
    {0x3000, 0x06},   // operating
    {DELAY_CODE, 500},
    {0x3000, 0x00},   // operating
    {DELAY_CODE, 500},
    {0x3008, 0x00},   //Master mode start
};
...
static u32 get_pclk(u32 xclk)
{
    ...
    isp_info("IMX178 Sensor: pclk(%d) XCLK(%d)\n", pclk, xclk);
...
static void calculate_t_row(void)
{
    ...
    if (g_psensor->img_win.width <= 1920 && g_psensor->img_win.height <= 1080)
        ...
        h_total = 3203;
    pinfo->t_row = (h_total * 10000) / (g_psensor->pclk / 1000);
...
static void adjust_blanking(int fps)
{
    ...
    write_reg(0x302c, (pinfo->VMax & 0xFF));
    write_reg(0x302d, ((pinfo->VMax >> 8) & 0xFF));
    write_reg(0x302e, ((pinfo->VMax >> 16) & 0x1));
...
//            mdelay(pInitTable[i].val);
    ...
    if (sensor_xclk == 54000000){
        write_reg(0x33be, 0x21); 
        write_reg(0x33bf, 0x21); 
        write_reg(0x33c0, 0x2c); 
        write_reg(0x33c1, 0x2c); 
        write_reg(0x33c2, 0x21); 
        write_reg(0x33c3, 0x2c); 
        write_reg(0x33c4, 0x16); 
        write_reg(0x33c5, 0x01); 
    }
    ...
    if (width<=1920 && height<=1080 ) {
        g_psensor->out_win.width = 1944;
        g_psensor->out_win.height = 1116;
    }
    else if (width<=2592 && height<=1944 ) {
        g_psensor->out_win.width = 2616;
        g_psensor->out_win.height = 1990;
    }
    else {    //3072x2048
        g_psensor->out_win.width = 3096;
        g_psensor->out_win.height = 2094;
    }
    g_psensor->img_win.x = ((g_psensor->out_win.width - width) / 2) & ~BIT0;
    g_psensor->img_win.y = ((g_psensor->out_win.height - height) / 2) & ~BIT0;
...
static u32 get_exp(void)
{
    ...
    if (!g_psensor->curr_exp) {
        read_reg(0x3034, &reg_l);
        read_reg(0x3035, &reg_h);
        read_reg(0x3036, &reg_msb);
...
static int set_exp(u32 exp)
{
    ...
    exp_line = MAX(8, (exp * 1000 + pinfo->t_row / 2 ) / pinfo->t_row);

    if (exp_line <= pinfo->VMax - 1) {
        shs1 = pinfo->VMax - exp_line;
        write_reg(0x3034, (shs1 & 0xFF));
        write_reg(0x3035, ((shs1 >> 8) & 0xFF));
        write_reg(0x3036, ((shs1 >> 16) & 0x1));
        if (pinfo->long_exp) {
            write_reg(0x302c, (pinfo->VMax & 0xFF));
            write_reg(0x302d, ((pinfo->VMax >> 8) & 0xFF));
            write_reg(0x302e, ((pinfo->VMax >> 16) & 0x1));
        ...
        exp_line += 1;
        write_reg(0x3034, 0x08);
        write_reg(0x3035, 0x00);
        write_reg(0x3036, 0x00);
        write_reg(0x302c, (exp_line & 0xFF));
        write_reg(0x302d, ((exp_line >> 8) & 0xFF));
        write_reg(0x302e, ((exp_line >> 16) & 0x1));
        pinfo->long_exp = 1;
    }
...
static int set_exp_us(u32 exp)
{
    ...
    exp_line = MAX(8, (exp * 10 + pinfo->t_row / 2 ) / pinfo->t_row);
...
static u32 get_gain(void)
{
    ...
    if (!g_psensor->curr_gain) {
        read_reg(0x301f, &reg_l);
        read_reg(0x3020, &reg_h);
...
static int set_gain(u32 gain)
{
    ...
    write_reg(0x301f, reg_l);
    write_reg(0x3020, reg_h);
...
// external functions
//=============================================================================
static void imx178_deconstruct(void)
{
...
static int imx178_construct(u32 xclk, u16 width, u16 height)
{
...
static int __init imx178_init(void)
{
    ...
    if ((ret = imx178_construct(sensor_xclk, sensor_w, sensor_h)) < 0)
        goto init_err2;
...
MODULE_DESCRIPTION("Sensor IMX178");
MODULE_LICENSE("GPL");
