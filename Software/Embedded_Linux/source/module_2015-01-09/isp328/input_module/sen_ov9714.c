...
//;30FPS KEY
    {0x3094,0x01},
    {0x30b3,0x0f}, // PLL_MUL,  0x0f= 45M , 0x10=48M,  0x14= 60M
    {0x4837,0x21},
//;
    {0x0100,0x01},
//;
...
static void calculate_t_row(void)
{
    ...
    pinfo->t_row =479;
    g_psensor->min_exp = 1;
    pinfo->sw_lines_max = 1125;
    pinfo->prev_sw_res = 100;*/
...
//    write_reg(0x3208, 0x00);    // Enable Group0
    if( fps > 30){
        scale = 2;
    ...
    //printk("pll2_multiplier=%d, sclk= %d, xclk= %d, pll2_prediv= %d, pll2_divs= %d, pll2_seld5= %d, \n", pll2_multiplier,
    //    pinfo->sclk,  g_psensor->xclk , pll2_prediv, pll2_divs , pll2_seld5);
...
//    write_reg(0x3208, 0x10);    // End Group0
//    write_reg(0x3208, 0xA0);    // Launch Group0

    //printk("hts=%d, vts=%d, fps=%d", hts, vts, fps);
...
static u32 get_gain(void)
{
    ...
    /*if (g_psensor->curr_gain == 0) {
        u32 reg_h, reg_l;
        u32 main_gain, fractional;
        read_reg(0x350A, &reg_h);
        read_reg(0x350B, &reg_l);
        main_gain = ((((reg_h & 0x3) << 4)) | ((reg_l>>4) & 0xF))*64;
        fractional = (reg_l & 0xF)*4;


        g_psensor->curr_gain = main_gain+fractional ;
    }*/

    //0x3503=7 (sensor gain)
    //manual; 1x~16x is analog gain. 16x~ 64x is digital gain,
    u32 gain_base, reg;
...
static int set_gain(u32 gain)
{
    ...
    write_reg(0x3208, 0x00);    // Enable Group0
    write_reg(0x350A, (gain >> 10) & 0x3);
    write_reg(0x350B, (gain >>2) & 0xFF);
    write_reg(0x3208, 0x10);    // End Group0
    write_reg(0x3208, 0xA0);    // Launch Group0
