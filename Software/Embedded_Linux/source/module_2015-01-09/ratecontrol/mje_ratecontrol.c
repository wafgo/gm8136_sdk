...
static void RcInit(MJFRC_statics * rc_st,
					...
					unsigned int reaction_delay_max)
{
    ...
    // Tuba 20110805_0 end
	if (MJFRateControlInit (rate_control,
					target_rate,
		...
		printk ("MJFRateControlReInit fail\n");
		return -1;
	}
	// keep deviation
...
skip_integrate_err:
	...
	if (quality_scale_fix >= scale) {
		...
		//base_quality_fix = mj_quality_fix_const + division(((base_quality_fix - mj_quality_fix_const) * quality_scale_fix),scale);
		base_quality_fix = (int64_t)mj_quality_fix_const*(int64_t)scale + ((base_quality_fix - mj_quality_fix_const) * quality_scale_fix);
	}
	...
	target_quality_fix =
		base_quality_fix + division((base_quality_fix - mj_quality_fix_const) * overflow_fix, rc_st->target_framesize_fix);
		
	RC_DEBUG("\t%d", (int)base_quality_fix);
	RC_DEBUG("\t%d", (int)target_quality_fix);
	RC_DEBUG("\t%d", mj_quality_fix_const);
	/*
	RC_DEBUG("\t%.3f", (float)base_quality_fix/scale);
	RC_DEBUG("\t%.5f", (float)target_quality_fix/scale);
	RC_DEBUG("\t%f", (float)mj_quality_fix_const/scale);*/
	/*
	...
	else if (target_quality_fix < mj_quality_fix_const)
		target_quality_fix = mj_quality_fix_const;

	rtn_quant = (int) division(scale_2, target_quality_fix);

	if (rtn_quant < mj_rc_MAX_QUANT) {
...
#endif
        DEBUG_M(LOG_DEBUG, "overflow %lld, quality %d -> %d (%d, %d)\n", fsz_delta, rtn_quant, rate_control->rtn_quant, q_delta_m, q_delta_p);
