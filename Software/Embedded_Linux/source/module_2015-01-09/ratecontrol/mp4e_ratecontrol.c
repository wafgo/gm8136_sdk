...
/*
	MPEG4 rate control fixed-point version:
	0.9:  Init. version
...
extern int under_converge_level;
extern int over_converge_level;
...
static void RcInit(MP4FRC_statics * rc_st,
...
static int mode(unsigned int target_rate, unsigned int target_rate_max)
{
	if (target_rate == 0) {
					...
					unsigned int reaction_delay_max)
{
	...
	*/
	...
	if ((max_quant < min_quant) || (initq < min_quant) || (max_quant < initq)) {
		...
		rate_control->rc_mode = mode(target_rate, target_rate_max);
		...
		RcInit (&rate_control->normal, target_rate, fincr, fbase);
		RcInit (&rate_control->max, target_rate_max, fincr, fbase);
					...
					unsigned int reaction_delay_max)
{
    ...
    // Tuba 20110805_0 end
	if (MP4FRateControlInit (rate_control,
					target_rate,
		...
		printk ("MP4FRateControlReInit fail\n");
		return -1;
	}
	// keep deviation
...
skip_integrate_err:
...
#else
    quality_scale_fix =
		division(scale * rc_st->target_framesize_fix , rc_st->avg_framesize_fix) * 
		division(rc_st->target_framesize_fix, rc_st->avg_framesize_fix);
#endif
	...
	if (quality_scale_fix >= scale) {
		...
		//base_quality_fix = mp4_quality_fix_const + division(((base_quality_fix - mp4_quality_fix_const) * quality_scale_fix),scale);
		base_quality_fix = (int64_t)mp4_quality_fix_const*(int64_t)scale + ((base_quality_fix - mp4_quality_fix_const) * quality_scale_fix);
	}
	...
	target_quality_fix =
		base_quality_fix + division((base_quality_fix - mp4_quality_fix_const) * overflow_fix, rc_st->target_framesize_fix);
		
	RC_DEBUG("\t%d", (int)base_quality_fix);
	RC_DEBUG("\t%d", (int)target_quality_fix);
	RC_DEBUG("\t%d", mp4_quality_fix_const);
	/*
	RC_DEBUG("\t%.3f", (float)base_quality_fix/scale);
	RC_DEBUG("\t%.5f", (float)target_quality_fix/scale);
	RC_DEBUG("\t%f", (float)mp4_quality_fix_const/scale);*/
	/*
	...
	else if (target_quality_fix < mp4_quality_fix_const)
		target_quality_fix = mp4_quality_fix_const;

	rtn_quant = (int) division(scale_2, target_quality_fix);

	if (rtn_quant < mp4_rc_MAX_QUANT) {
	...
	RC_DEBUG("\t%d", rtn_quant);
	{
		...
		if (fsz_delta < 0){
        #ifdef FAST_CONVERGE
            if (under_converge_level) {
                int underflow_level;
                underflow_level = (int)division(fsz_delta, (rc_st->target_framesize_fix/2));
                underflow_level = RC_CLIP3(underflow_level, -1*under_converge_level, -1);
                q_delta_m = underflow_level;
            }
            else
                q_delta_m =  -1;
            /*
        ...
        #ifdef FAST_CONVERGE
            if (over_converge_level) {
                int overflow_level;
                overflow_level = (int)division(fsz_delta, (rc_st->target_framesize_fix/2));
                overflow_level = RC_CLIP3(overflow_level, 1, over_converge_level);
                q_delta_p = overflow_level;
            }
            else
                q_delta_p = 1;
            /*
