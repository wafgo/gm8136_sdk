/*
 * Revision History
 * VER0.1.0 2014/03/03 Add SWOSG8139 VERSION 
 * VER0.1.1 2014/03/20 sync from 8210 
 * VER0.1.2 2014/03/31 add get canvas info and printk limit
 * VER0.1.3 2014/04/03 Add some function for static mode , but not support
 * VER0.1.4 2014/04/18 Add some function for static mode , but not support
 * VER0.1.5 2014/05/08 2 pixsel instead of 4 pixsel 
 * VER0.1.6 2014/05/12 add static memory allocate mode 
 * VER0.1.7 2014/05/14 change flush dcache funtion 
 */
...
#define SWOSG8139_VERSION   ((0 << 16) + (1 << 8) + 7) 
...
int swosg_eng_canvas_set( int id, swosg_if_canvas_t * canvas)
{
    ...
    if(id >= SWOSG_CANVAS_MAX || id < 0){
        DEBUG_M(LOG_WARNING,"canvas_set id out of range(%d)\n",id);    
        return -1;
    }
    ...
    if(g_swosg_eng_canvas_array[id].usage == 0)
    {
        ...
        canvas_dat->canvas_vaddr = kmalloc(canvas_dat->canvas_size, GFP_KERNEL);
        if(!canvas_dat->canvas_vaddr){
...
int swosg_eng_canvas_del( int id)
{
    ...
    if(id >= SWOSG_CANVAS_MAX || id < 0){
        DEBUG_M(LOG_WARNING,"canvas_del id out of range(%d)\n",id);    
        return -1;
    }
...
{   
    ...
    unsigned char* src_imgdata;
    unsigned int dx,dy,dw,dh,dpitch,spitch,s_yuv,byte_sz;
    unsigned int *src_data, *src_line;
    unsigned char *dst_data, *dst_line;
    unsigned int i,j;
    //unsigned int flush_addr,delat_b,byte_sz1;
    ...
    if(canvas_info->canvas_vaddr != NULL)
        src_imgdata =(unsigned char*) canvas_info->canvas_vaddr;
    else{
    ...
    dpitch = blit_param->dest_bg_w<<1;
    spitch = canvas_info->canvas_w>>1;
    ...
    dst_line = (unsigned char*)&imgdata[(dy*dpitch)+(dx<<1)];
    src_line = (unsigned int *)&src_imgdata[0];
    ...
    for(i = 0 ; i < dh ; i++){
        ...
        for(j = 0 ; j < dw ; j+=2){
            ...
            if((s_yuv & 0xff000000) != 0xff000000) { /*yvyu*/
               if((s_yuv & 0x0000ff00) != 0x0000ff00)
                  *(unsigned int *)dst_data = s_yuv;
               else
                  *(dst_data+3) = (char)(s_yuv >>24);
            }
            else if((s_yuv & 0x0000ff00) != 0x0000ff00)
                  *(dst_data+1) = (char)(s_yuv >>8);            
    ...
    if(fire){
        ...
        if(frm_get_addrinfo(&fram_buf_in,0)){
            DEBUG_M(LOG_WARNING,"blit dest_bg_addr isn't from frammap(%x)\n",blit_param->dest_bg_addr);
            return -1;    
        }
        
        if(blit_param->dest_bg_addr%DMA_CACHE_LINE_SZ){
            DEBUG_M(LOG_WARNING,"blit dest_bg_addr isn't align cache size(%x)\n",blit_param->dest_bg_addr);
            return -1;    
        }    
        ...
        //__cpuc_flush_dcache_area((void*)blit_param->dest_bg_addr, byte_sz);
        fmem_dcache_sync((void*)blit_param->dest_bg_addr, byte_sz, DMA_TO_DEVICE);
    }
#else
    flush_addr = (unsigned int)&imgdata[((dy)*dpitch)+((dx)*2)];
    delat_b = (flush_addr % DMA_CACHE_LINE_SZ );
    flush_addr = flush_addr - (flush_addr % DMA_CACHE_LINE_SZ );

    if(flush_addr < blit_param->dest_bg_addr){
         __cpuc_flush_dcache_area((void*)blit_param->dest_bg_addr, byte_sz);
         DEBUG_M(LOG_DEBUG,"all __cpuc_flush_dcache_area %x %x\n",flush_addr,blit_param->dest_bg_addr);
         goto flush_done;   
    }
        
    byte_sz1 = ((dh-1)*dpitch) + ((blit_param->dest_bg_w-dx)*2)+ delat_b ;
    DEBUG_M(LOG_DEBUG,"original add=%x flush_a=%x ,delt=%u,byte_sz1=%u\n",blit_param->dest_bg_addr
           ,flush_addr,delat_b,byte_sz1);
    
    byte_sz1 += (byte_sz1 % DMA_CACHE_LINE_SZ);

    if((flush_addr + byte_sz1) > (blit_param->dest_bg_addr +byte_sz)){
         __cpuc_flush_dcache_area((void*)blit_param->dest_bg_addr, byte_sz);
         DEBUG_M(LOG_DEBUG,"all __cpuc_flush_dcache_area %x %x\n",(flush_addr+byte_sz1),blit_param->dest_bg_addr+byte_sz);
         goto flush_done;     
    }
    
    DEBUG_M(LOG_DEBUG,"final flush_addr=%x size=%u\n",flush_addr,byte_sz1);
    __cpuc_flush_dcache_area((void*)flush_addr, byte_sz1);
...
{
    unsigned int x1,y1,x2,y2,i,j,border_s = 0,byte_sz;
    struct swosg_eng_point startPoint ,endpoint;
    swosg_if_mask_param_t *tmp_param;
    frammap_buf_t         fram_buf_in;
    //unsigned int          flush_addr,delat_b, dpitch,byte_sz1;
    ...
    for(j = 0 ; j < num ; j++){
        ...
        tmp_param = &mask_param[j];
        
        if (tmp_param->dest_bg_w <= 0 || tmp_param->dest_bg_h <= 0 
            ||tmp_param->target_w == 0 ||  tmp_param->target_h ==0)                                                           
        {
            DEBUG_M(LOG_WARNING,"swosg:rect range is error target w-h(%d-%d) dest w-h(%d-%d)\n"
                   ,tmp_param->target_w ,tmp_param->target_h,tmp_param->dest_bg_w, tmp_param->dest_bg_h);
            continue;                                                                          
        }

        if((tmp_param->target_x + tmp_param->target_w) > tmp_param->dest_bg_w  || 
           (tmp_param->target_y + tmp_param->target_h) > tmp_param->dest_bg_h )
        {
            DEBUG_M(LOG_WARNING,"swosg:rect param is error tar x-y(%d-%d) tar w-h(%d-%d) dest w-h(%d-%d)\n"
                ,tmp_param->target_x,tmp_param->target_y,tmp_param->target_w,tmp_param->target_h
                ,tmp_param->dest_bg_w, tmp_param->dest_bg_h);
			continue;
        }

        if(swosg_eng_check_border(tmp_param) < 0){
            DEBUG_M(LOG_WARNING,"swosg:check border is over range tar x-y-w-h(%d-%d-%d-%d) border sz (%d)\n"
                     ,tmp_param->target_x,tmp_param->target_y,tmp_param->target_w,tmp_param->target_h,tmp_param->border_size);
            continue;
        }

        if(tmp_param->align_type >= SWOSG_WIN_ALIGN_MAX){
            DEBUG_M(LOG_WARNING,"swosg:check alogn type is over range (%d)\n",tmp_param->align_type);
            continue;
        }        

        if(swosg_eng_get_align_mask_xy(tmp_param,&x1,&y1) < 0){
            DEBUG_M(LOG_WARNING,"swosg:get alignxy type(%d)is error tar x-y-w-h(%d-%d-%d-%d) dest w-h(%d-%d)\n"
                ,tmp_param->align_type,tmp_param->target_x,tmp_param->target_y,tmp_param->target_w,tmp_param->target_h
                ,tmp_param->dest_bg_w,tmp_param->dest_bg_h);
		    continue;        
        }
        
		if(tmp_param->palette_idx >= SWOSG_PALETTE_MAX){
            DEBUG_M(LOG_WARNING,"osg:%s palette index is over %d\n",__func__,tmp_param->palette_idx);
            continue;
    	}
        if(tmp_param->border_type == SWOSGIF_BORDER_TURE){
            x1 = x1 + tmp_param->border_size;
            y1 = y1 + tmp_param->border_size;
            x2 = (x1+ tmp_param->target_w - 1)- tmp_param->border_size;
            y2 = (y1 + tmp_param->target_h - 1)- tmp_param->border_size;  
            border_s = y2-y1 +1;
        }
        else if(tmp_param->border_type == SWOSGIF_BORDER_HORROW){
            x1 = x1 ;
            y1 = y1 ;
            x2 = x1 + tmp_param->target_w - 1 ;
            y2 = y1 + tmp_param->target_h - 1;
            border_s = (tmp_param->border_size == 0) ? 1: tmp_param->border_size;            
        }
        else{
            tmp_param->border_type = SWOSGIF_BORDER_HORROW;
            x1 = x1 ;
            y1 = y1 ;
            x2 = x1 + tmp_param->target_w - 1 ;
            y2 = y1 + tmp_param->target_h - 1;
        ...
        DEBUG_M(LOG_DEBUG,"%s rect xy=%dx%d wh=%dx%d add=%x size=%x type=%d\n",__func__, x1, \
                y1,x2,y2,(unsigned int)tmp_param->dest_bg_addr,border_s,tmp_param->border_type);

        for( i = 0 ; i < border_s ; i++){
            // horizontal line at top
            if( tmp_param->border_type == SWOSGIF_BORDER_HORROW){
                startPoint.x = x1 , startPoint.y = y1+i;
                endpoint.x = x2 , endpoint.y = y1+i;
                if(swosg_eng_draw_line_one_frame(&startPoint,&endpoint,tmp_param)<0){
                    DEBUG_M(LOG_ERROR,"osg:osg_draw_line_one_frame is error %d\n",__LINE__);
                    continue;
                }
              
            }else if(tmp_param->border_type == SWOSGIF_BORDER_TURE){
                startPoint.x = x1 , startPoint.y = y1+i;
                endpoint.x = x2 , endpoint.y = y1+i;        
                if(swosg_eng_draw_line_true_frame(&startPoint,&endpoint,tmp_param)<0){
                    DEBUG_M(LOG_ERROR,"osg:osg_draw_line_true_frame is error %d\n",__LINE__);
                    continue;
                }
        ...
        if(j == (num-1)){
            ...
            fram_buf_in.va_addr = (void*)tmp_param->dest_bg_addr;  
        
            if(frm_get_addrinfo(&fram_buf_in,0)){
                DEBUG_M(LOG_WARNING,"mask dest_bg_addr isn't from frammap(%x)\n",tmp_param->dest_bg_addr);
                goto mask_err;    
            }
        
            if(tmp_param->dest_bg_addr%DMA_CACHE_LINE_SZ){
                DEBUG_M(LOG_WARNING,"mask dest_bg_addr isn't align cache size(%x)\n",tmp_param->dest_bg_addr);
                goto mask_err; 
            }    
        
            byte_sz = (tmp_param->dest_bg_w*tmp_param->dest_bg_h*2);
        
            if(byte_sz%DMA_CACHE_LINE_SZ){
                DEBUG_M(LOG_WARNING,"mask dest_bg_size isn't align cache size(%d)\n",byte_sz);
                goto mask_err;    
            }    
            //__cpuc_flush_dcache_area((void*)tmp_param->dest_bg_addr, byte_sz);
            fmem_dcache_sync((void*)tmp_param->dest_bg_addr, byte_sz, DMA_TO_DEVICE);
        }
#else
        dpitch = tmp_param->dest_bg_w *2;
        flush_addr = (unsigned int)tmp_param->dest_bg_addr + ((y1)*dpitch)+((x1)*2);
        delat_b = (flush_addr % DMA_CACHE_LINE_SZ );
        flush_addr = flush_addr - (flush_addr % DMA_CACHE_LINE_SZ );

        if(flush_addr < tmp_param->dest_bg_addr){
            __cpuc_flush_dcache_area((void*)tmp_param->dest_bg_addr, byte_sz);
            DEBUG_M(LOG_DEBUG,"mask < __cpuc_flush_dcache_area %x %x\n",flush_addr,tmp_param->dest_bg_addr);
            continue;
        }
            
        byte_sz1 = ((tmp_param->target_h-1)*dpitch) + ((tmp_param->dest_bg_w-x1)*2)+ delat_b ;
        
       
        DEBUG_M(LOG_DEBUG,"mask original add=%x flush_a=%x ,delt=%u,byte_sz1=%u\n",tmp_param->dest_bg_addr
               ,flush_addr,delat_b,byte_sz1);

        byte_sz1 += (byte_sz1 % DMA_CACHE_LINE_SZ);

        if((flush_addr + byte_sz1) > (tmp_param->dest_bg_addr +byte_sz)){
            __cpuc_flush_dcache_area((void*)tmp_param->dest_bg_addr, byte_sz);
            DEBUG_M(LOG_DEBUG,"mask > __cpuc_flush_dcache_area %x %x\n",(flush_addr+byte_sz1),tmp_param->dest_bg_addr+byte_sz);
            continue;
        }

        DEBUG_M(LOG_DEBUG,"mask final flush_addr=%x size=%u\n",flush_addr,byte_sz1);
        __cpuc_flush_dcache_area((void*)flush_addr, byte_sz1);
#endif        
    }
    swosg_eng_unlock_canvas_mutex();
