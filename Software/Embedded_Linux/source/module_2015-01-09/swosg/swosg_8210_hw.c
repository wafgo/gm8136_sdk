 ...
 * VER0.2.2 2014/03/18 support hw for 8210 
 * VER0.2.3 2014/03/27 support remiander command fire when last item was fail
 * VER0.2.4 2014/03/28 add more debug message
 * VER0.2.5 2014/03/31 add more debug message and printk limit
 * VER0.2.6 2014/04/03 suooprt static canvas pool for DH
 * VER0.2.7 2014/04/16 enlarge static canvas pool number to 68 , and avoid 
 * error return when this static pool memory length is zero. 
 * VER0.2.8 2014/04/18 add swosg_eng_canvas_get_static_pool function.
 * VER0.2.9 2014/05/12 modify static mem pool interface 
 */
...
#define SWOSG8210_VERSION     ((0 << 16) + (2 << 8) + 9) 
...
dest_cav->canvas_size);
    ...
    if (!((unsigned int)dest_cav->canvas_vaddr%DMA_CACHE_LINE_SZ)) {
        __cpuc_flush_dcache_area((void*)dest_cav->canvas_vaddr, dest_cav->
canvas_size);
...
{   
    ...
    ft2d_osg_blit_t  ft2d_blit_data;
#if 1 /*8210 dest was flush by last job*/   
    ...
    memset(&ft2d_blit_data,0x0,sizeof(ft2d_osg_blit_t));
    ...
    if(frm_get_addrinfo(&fram_buf_in,0) == 0){
        ft2d_blit_data.dst_paddr = (unsigned int)frm_va2pa(blit_param->dest_bg_addr);        
    }else{
    ...
    ft2d_blit_data.dst_w = blit_param->dest_bg_w;
    ft2d_blit_data.dst_h = blit_param->dest_bg_h;
    ft2d_blit_data.src_paddr = (unsigned int)canvas_info->canvas_paddr;
    ft2d_blit_data.src_w = canvas_info->canvas_w;
    ft2d_blit_data.src_h = canvas_info->canvas_h;
    ft2d_blit_data.dx    = dx;
    ft2d_blit_data.dy    = dy;
    ft2d_blit_data.src_colorkey = COLOUR565(0xff,0xff,0xff);
   
    if(ft2d_osg_do_blit(&ft2d_blit_data,fire) < 0){        
        DEBUG_M(LOG_WARNING,"ft2d_osg_do_blit is error\n");
        return -1;
    }    
    return 0;
...
{   
    ...
    memset(&t2d_osgmask,0x0,sizeof(ft2d_osg_mask_t));
#if 1 /*8210 dest was flush by last job*/   
    memset(&fram_buf_in,0x0,sizeof(frammap_buf_t));
    ...
    t2d_osgmask.color     = OSG_GET_PALETEE(mask_param->palette_idx)&0xffff;
    t2d_osgmask.type      = mask_param->border_type;

    if(ft2d_osg_do_mask(&t2d_osgmask,fire) < 0){        
