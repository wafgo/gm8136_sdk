/**
 * @file nvp1104.c
 * VCAP300 NVP1104 Input Device Driver
 * Copyright (C) 2012 GM Corp. (http://www.grain-media.com)
 *
 * $Revision: 1.15 $
 * $Date: 2014/07/14 03:22:15 $
 *
 * ChangeLog:
 *  $Log: nvp1104.c,v $
 *  Revision 1.15  2014/07/14 03:22:15  jerson_l
 *  1. update for support dynamic switch video mode
 *
 *  Revision 1.14  2014/03/31 02:33:06  jerson_l
 *  1. remove un-used vcap_i2c.h header file include
 *
 *  Revision 1.13  2014/02/06 02:58:22  jerson_l
 *  1. modify version compare rule of input module driver
 *
 *  Revision 1.12  2014/02/06 02:14:04  jerson_l
 *  1. record channel vlos status when status notified
 *
 *  Revision 1.11  2013/11/27 05:38:47  jerson_l
 *  1. support status notify
 *
 *  Revision 1.10  2013/11/07 06:40:00  jerson_l
 *  1. support max_frame_rate infomation
 *
 *  Revision 1.9  2013/10/01 06:18:19  jerson_l
 *  1. support new channel mapping mechanism
 *
 *  Revision 1.8  2013/09/12 11:39:27  jerson_l
 *  1. switch default order to odd field first
 *
 *  Revision 1.7  2013/08/23 09:18:07  jerson_l
 ...
 *
 *  Revision 1.5  2013/01/31 05:47:40  jerson_l
 *  1. modify for use export library of av_decoder/nvp1104 driver to control nvp1104
 *
 *  Revision 1.4  2013/01/15 09:40:42  jerson_l
 *  1. add vi_src and data_swap for capture port pinmux control
 *
 *  Revision 1.3  2012/12/11 09:20:05  jerson_l
...
#define DEV_MAX                 NVP1104_DEV_MAX
#define DEV_VPORT_MAX           NVP1104_DEV_VPORT_MAX            ///< VPORT_A, VPORT_B, VPORT_C, VPORT_D
#define DEV_CH_MAX              NVP1104_DEV_CH_MAX
...
MODULE_PARM_DESC(norm, "Video Norm=> 0:PAL, 1:NTSC, 2:CIF_PAL, 3:CIF_NTSC");
...
static struct nvp1104_dev_t   nvp1104_dev[DEV_MAX];
static struct proc_dir_entry *nvp1104_proc_root[DEV_MAX]    = {[0 ... (DEV_MAX - 1)] = NULL};
static struct proc_dir_entry *nvp1104_proc_norm[DEV_MAX]    = {[0 ... (DEV_MAX - 1)] = NULL};
static struct proc_dir_entry *nvp1104_proc_timeout[DEV_MAX] = {[0 ... (DEV_MAX - 1)] = NULL};

/****************************************************************************************
 *  NVP1104 VPort and X_CAP# Port Mapping for GM8210 EVB
 *
 *  NVP1104#0(0x70) --->VPortA ---- [VIN#0,1,2,3] 108MHz ---> X_CAP#0
 *                  --->VPortB ---- X
 *                  --->VPortC ---- X
 *                  --->VPortD ---- X
 *
 *  NVP1104#1(0x72) --->VPortA ---- [VIN#0,1,2,3] 108MHz ---> X_CAP#1
 *                  --->VPortB ---- X
 *                  --->VPortC ---- X
 *                  --->VPortD ---- X
 *
 *  NVP1104#1(0x74) --->VPortA ---- [VIN#0,1,2,3] 108MHz ---> X_CAP#2
 *                  --->VPortB ---- X
 *                  --->VPortC ---- X
 *                  --->VPortD ---- X
 *
 *  NVP1104#1(0x76) --->VPortA ---- [VIN#0,1,2,3] 108MHz ---> X_CAP#3
 *                  --->VPortB ---- X
 *                  --->VPortC ---- X
 *                  --->VPortD ---- X
 *****************************************************************************************/

static int nvp1104_module_get(void)
{
    return try_module_get(THIS_MODULE);
}

static void nvp1104_module_put(void)
{
    module_put(THIS_MODULE);
}

static int nvp1104_device_init(int id, int norm)
{
    int i, ret = 0;
    ...
    if((id >= DEV_MAX) || (id >= nvp1104_get_device_num()))
        return -1;

    if(nvp1104_dev[id].norm != norm) {
        switch(norm) {
            case VCAP_INPUT_NORM_PAL:
                ...
                switch(mode[id]) {
                    case 1:  /* 54MHz ==> 27MHz Clock with 54MHz Data */
                        vmode     = NVP1104_VMODE_PAL_720H_2CH;
                        pixel_clk = 27000000;
                        break;
                    case 2: /* 108MHz */
                        vmode     = NVP1104_VMODE_PAL_720H_4CH;
                        pixel_clk = 108000000;
                        break;
                    default: /* 27MHz */
                        vmode     = NVP1104_VMODE_PAL_720H_1CH;
                        pixel_clk = 27000000;
                        break;
                }
                break;
            case 2: /* 4CIF_PAL */
                ...
                pixel_clk  = 54000000;
                timeout    = DEFAULT_PAL_TIMEOUT;
                vmode      = NVP1104_VMODE_PAL_CIF_4CH;
                break;
            case 3: /* 4CIF_NTSC */
                width      = 360;
                height     = 480;
                ...
                vmode      = NVP1104_VMODE_NTSC_CIF_4CH;
                break;
            default:    /* NTSC */
                width      = 720;
                height     = 480;
                ...
                switch(mode[id]) {
                    case 1:  /* 54MHz ==> 27MHz Clock with 54MHz Data */
                        vmode     = NVP1104_VMODE_NTSC_720H_2CH;
                        pixel_clk = 27000000;
                        break;
                    case 2: /* 108MHz */
                        vmode     = NVP1104_VMODE_NTSC_720H_4CH;
                        pixel_clk = 108000000;
                        break;
                    default: /* 27MHz */
                        vmode     = NVP1104_VMODE_NTSC_720H_1CH;
                        pixel_clk = 27000000;
                        break;
                }
                break;
        ...
        /* Update device norm */
        nvp1104_dev[id].norm  = norm;
        nvp1104_dev[id].vmode = vmode;
        for(i=0; i<DEV_VPORT_MAX; i++) {
            pinput = nvp1104_dev[id].port[i];
    ...
    if(ret < 0)
        vcap_err("NVP1104#%d init failed!\n", id);

    return ret;
}

static int nvp1104_proc_norm_show(struct seq_file *sfile, void *v)
{
...
static int nvp1104_proc_timeout_show(struct seq_file *sfile, void *v)
{
    int i;
    u32 time_ms = 0;
    struct nvp1104_dev_t *pdev = (struct nvp1104_dev_t *)sfile->private;
...
static ssize_t nvp1104_proc_norm_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    int  ret = 0;
    int  norm_value;
    char value_str[8] = {'\0'};
    ...
    if((norm_value <= 3) && (norm_value != norm[pdev->index])) {
        ret = nvp1104_device_init(pdev->index, norm_value);
        if(ret < 0) {
            ret = -EFAULT;
            goto end;
        }
...
static ssize_t nvp1104_proc_timeout_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    int  i, time_value;
    char value_str[32] = {'\0'};
    struct seq_file *sfile = (struct seq_file *)file->private_data;
...
static int nvp1104_proc_norm_open(struct inode *inode, struct file *file)
{
    return single_open(file, nvp1104_proc_norm_show, PDE(inode)->data);
}

static int nvp1104_proc_timeout_open(struct inode *inode, struct file *file)
{
    return single_open(file, nvp1104_proc_timeout_show, PDE(inode)->data);
}

static struct file_operations nvp1104_proc_norm_ops = {
    .owner  = THIS_MODULE,
    .open   = nvp1104_proc_norm_open,
    .write  = nvp1104_proc_norm_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations nvp1104_proc_timeout_ops = {
    .owner  = THIS_MODULE,
    .open   = nvp1104_proc_timeout_open,
    .write  = nvp1104_proc_timeout_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};
...
static void nvp1104_proc_remove(int id)
{
    if(nvp1104_proc_root[id]) {
        if(nvp1104_proc_timeout[id])
            vcap_input_proc_remove_entry(nvp1104_proc_root[id], nvp1104_proc_timeout[id]);

        if(nvp1104_proc_norm[id])
            vcap_input_proc_remove_entry(nvp1104_proc_root[id], nvp1104_proc_norm[id]);

        vcap_input_proc_remove_entry(NULL, nvp1104_proc_root[id]);
    }
}

static int nvp1104_proc_init(struct nvp1104_dev_t *pdev)
{
    int ret = 0;
    int id  = pdev->index;
    char name[32];
    ...
    /* root */
    sprintf(name, "nvp1104.%d", id);
    nvp1104_proc_root[id] = vcap_input_proc_create_entry(name, S_IFDIR|S_IRUGO|S_IXUGO, NULL);
    if(!nvp1104_proc_root[id]) {
    ...
    /* norm */
    nvp1104_proc_norm[id] = vcap_input_proc_create_entry("norm", S_IRUGO|S_IXUGO, nvp1104_proc_root[id]);
    if(!nvp1104_proc_norm[id]) {
        vcap_err("create proc node '%s/norm' failed!\n", name);
        ret = -EINVAL;
        goto err;
    }
    nvp1104_proc_norm[id]->proc_fops = &nvp1104_proc_norm_ops;
    nvp1104_proc_norm[id]->data      = (void *)pdev;
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,30))
    nvp1104_proc_norm[id]->owner     = THIS_MODULE;
#endif

    /* timeout */
    nvp1104_proc_timeout[id] = vcap_input_proc_create_entry("timeout", S_IRUGO|S_IXUGO, nvp1104_proc_root[id]);
    if(!nvp1104_proc_timeout[id]) {
        vcap_err("create proc node '%s/timeout' failed!\n", name);
        ret = -EINVAL;
        goto err;
    }
    nvp1104_proc_timeout[id]->proc_fops = &nvp1104_proc_timeout_ops;
    nvp1104_proc_timeout[id]->data      = (void *)pdev;
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,30))
    nvp1104_proc_timeout[id]->owner     = THIS_MODULE;
#endif
...
static int nvp1104_vlos_notify_handler(int id, int vin, int vlos)
{
    int i, j, vch;
    struct nvp1104_dev_t *pdev = &nvp1104_dev[id];
...
static int nvp1104_vfmt_notify_handler(int id, int vmode)
{
    ...
    if(pdev->vmode != vmode) {
        ...
        ret = nvp1104_device_init(id, input_norm);
        if(ret == 0) {
            for(i=0; i<DEV_VPORT_MAX; i++) {
                if(!pdev->port[i])
                    continue;
...
static int __init nvp1104_input_init(void)
{
    int i, j;
    int dev_num;
    struct vcap_input_dev_t *pinput;
    ...
    /* clear buffer */
    memset(nvp1104_dev, 0, sizeof(struct nvp1104_dev_t)*DEV_MAX);

    /* Get NVP1104 Device Number */
    dev_num = nvp1104_get_device_num();

    for(i=0; i<DEV_MAX; i++) {
        nvp1104_dev[i].index = i;
        nvp1104_dev[i].norm  = -1;  ///< init value
        ...
        if(vport[i]) {
            ...
            for(j=0; j<DEV_VPORT_MAX; j++) {
                if((vport[i]>>(4*j)) & 0xf) {   ///< VPort# Connect to X_CAP#
                    pinput = nvp1104_dev[i].port[j] = kzalloc(sizeof(struct vcap_input_dev_t), GFP_KERNEL);
                    if(pinput == NULL) {
                        vcap_err("NVP1104#%d.%d allocate vcap_input_dev_t failed\n", i, j);
                        ret = -ENOMEM;
                        goto err;
                    }

                    /* input device name */
                    snprintf(pinput->name, VCAP_INPUT_NAME_SIZE-1, "nvp1104.%d.%d", i, j);
                    ...
                    pinput->init        = nvp1104_device_init;
                    pinput->norm.mode   = -1;                            ///< init value
                    pinput->module_get  = nvp1104_module_get;
                    pinput->module_put  = nvp1104_module_put;

                    switch(mode[i]) {
                        case 0: /* 27MHz */
                            pinput->mode     = VCAP_INPUT_MODE_BYPASS;
                            pinput->ch_id[0] = nvp1104_get_vch_id(i, j, 0);
                            break;
                        case 1:  /* 54MHz ==> 27MHz Clock with 54MHz Data */
                            pinput->mode     = VCAP_INPUT_MODE_2CH_BYTE_INTERLEAVE;
                            pinput->ch_id[0] = nvp1104_get_vch_id(i, j, 0);
                            pinput->ch_id[2] = nvp1104_get_vch_id(i, j, 1);
                            break;
                        default: /* 108MHz or 54MHz_4CIF */
                            pinput->mode     = VCAP_INPUT_MODE_4CH_BYTE_INTERLEAVE;
                            pinput->ch_id[0] = nvp1104_get_vch_id(i, j, 0);
                            pinput->ch_id[1] = nvp1104_get_vch_id(i, j, 1);
                            pinput->ch_id[2] = nvp1104_get_vch_id(i, j, 2);
                            pinput->ch_id[3] = nvp1104_get_vch_id(i, j, 3);
                            break;
                    }
                    ...
                    if(ret < 0) {
                        vcap_err("register NVP1104#%d.%d input device failed\n", i, j);
                        goto err;
                    }
                }
            }
            ...
            /* device proc init */
            ret = nvp1104_proc_init(&nvp1104_dev[i]);
            if(ret < 0)
                goto err;

            /* device init */
            ret = nvp1104_device_init(i, norm[i]);
            if(ret < 0)
                goto err;

            /* register nvp1104 video loss notify handler */
            nvp1104_notify_vlos_register(i, nvp1104_vlos_notify_handler);

            /* register nvp1104 video format notify handler */
            nvp1104_notify_vfmt_register(i, nvp1104_vfmt_notify_handler);

            vcap_info("Register NVP1104#%d Input Device\n", i);
        }
    }
...
err:
    for(i=0; i<DEV_MAX; i++) {
        nvp1104_notify_vlos_deregister(i);
        nvp1104_notify_vfmt_deregister(i);
        nvp1104_proc_remove(i);
        for(j=0; j<DEV_VPORT_MAX; j++) {
            if(nvp1104_dev[i].port[j]) {
                vcap_input_device_unregister(nvp1104_dev[i].port[j]);
                kfree(nvp1104_dev[i].port[j]);
            }
        }
    }
    return ret;
...
static void __exit nvp1104_input_exit(void)
{
    int i, j;
...
module_init(nvp1104_input_init);
module_exit(nvp1104_input_exit);

MODULE_DESCRIPTION("Grain Media Video Capture300 NVP1104 Input Driver");
