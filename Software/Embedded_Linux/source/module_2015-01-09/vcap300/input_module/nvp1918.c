/**
 * @file nvp1918.c
 * VCAP300 NVP1918 Input Device Driver
 * Copyright (C) 2012 GM Corp. (http://www.grain-media.com)
 *
 * $Revision: 1.13 $
 * $Date: 2014/06/17 03:24:21 $
 *
 * ChangeLog:
 *  $Log: nvp1918.c,v $
 *  Revision 1.13  2014/06/17 03:24:21  jerson_l
 ...
 *
 *  Revision 1.1  2013/01/31 05:42:05  jerson_l
 *  1. add nvp1918 decoder support
 *
 */
...
#include "vcap_dbg.h"
#include "nextchip/nvp1918.h"   ///< from module/include/front_end

#define DEV_MAX                 NVP1918_DEV_MAX
#define DEV_VPORT_MAX           NVP1918_DEV_VPORT_MAX            ///< VPORT_A, VPORT_B, VPORT_C, VPORT_D
#define DEV_CH_MAX              NVP1918_DEV_CH_MAX
...
static struct nvp1918_dev_t   nvp1918_dev[DEV_MAX];
static struct proc_dir_entry *nvp1918_proc_root[DEV_MAX]    = {[0 ... (DEV_MAX - 1)] = NULL};
static struct proc_dir_entry *nvp1918_proc_norm[DEV_MAX]    = {[0 ... (DEV_MAX - 1)] = NULL};
static struct proc_dir_entry *nvp1918_proc_timeout[DEV_MAX] = {[0 ... (DEV_MAX - 1)] = NULL};

/****************************************************************************************
 *  NVP1918 VPort and X_CAP# Port Mapping for GM8210/GM8287 Socket Board
 *
 *  NVP1918#0(0x66) --->VPortA ---- [VIN#0,1,2,3] 108/144MHz ---> X_CAP#1
 *                  --->VPortB ---- [VIN#4,5,6,7] 108/144MHz ---> X_CAP#0
 *                  --->VPortC ---- X
 *                  --->VPortD ---- X
 *
 *  NVP1918#1(0x64) --->VPortA ---- [VIN#0,1,2,3] 108/144MHz ---> X_CAP#3
 *                  --->VPortB ---- [VIN#4,5,6,7] 108/144MHz ---> X_CAP#2
 *                  --->VPortC ---- X
 *                  --->VPortD ---- X
 *
 *  NVP1918#2(0x62) --->VPortA ---- [VIN#0,1,2,3] 108/144MHz ---> X_CAP#5
 *                  --->VPortB ---- [VIN#4,5,6,7] 108/144MHz ---> X_CAP#4
 *                  --->VPortC ---- X
 *                  --->VPortD ---- X
 *
 *  NVP1918#3(0x60) --->VPortA ---- [VIN#0,1,2,3] 108/144MHz ---> X_CAP#7
 *                  --->VPortB ---- [VIN#4,5,6,7] 108/144MHz ---> X_CAP#6
 *                  --->VPortC ---- X
 *                  --->VPortD ---- X
 *========================================================================================
 *  NVP1918 VPort and X_CAP# Port Mapping for GM8210 System Board
 *
 *  NVP1918#0(0x66) --->VPortA ---- [VIN#0,1,2,3] 108/144MHz ---> X_CAP#0
 *                  --->VPortB ---- [VIN#4,5,6,7] 108/144MHz ---> X_CAP#1
 *                  --->VPortC ---- X
 *                  --->VPortD ---- X
 *
 *  NVP1918#1(0x64) --->VPortA ---- [VIN#0,1,2,3] 108/144MHz ---> X_CAP#2
 *                  --->VPortB ---- [VIN#4,5,6,7] 108/144MHz ---> X_CAP#3
 *                  --->VPortC ---- X
 *                  --->VPortD ---- X
 *
 *  NVP1918#2(0x62) --->VPortA ---- [VIN#0,1,2,3] 108/144MHz ---> X_CAP#4
 *                  --->VPortB ---- [VIN#4,5,6,7] 108/144MHz ---> X_CAP#5
 *                  --->VPortC ---- X
 *                  --->VPortD ---- X
 *
 *  NVP1918#3(0x60) --->VPortA ---- [VIN#0,1,2,3] 108/144MHz ---> X_CAP#6
 *                  --->VPortB ---- [VIN#4,5,6,7] 108/144MHz ---> X_CAP#7
 *                  --->VPortC ---- X
 *                  --->VPortD ---- X
 *****************************************************************************************/

static int nvp1918_module_get(void)
{
    return try_module_get(THIS_MODULE);
}

static void nvp1918_module_put(void)
{
    module_put(THIS_MODULE);
}

static int nvp1918_device_init(int id, int norm)
{
    int i, ret = 0;
    ...
    if((id >= DEV_MAX) || (id >= nvp1918_get_device_num()))
        return -1;

    if(nvp1918_dev[id].norm != norm) {
        switch(norm) {
            case VCAP_INPUT_NORM_PAL:
                ...
                switch(mode[id]) {
                    case 0:  /* 36MHz */
                        vmode     = NVP1918_VMODE_PAL_720H_1CH;
                        pixel_clk = 36000000;
                        break;
                    case 1:  /* 72MHz ==> 36MHz Clock with 72MHz Data */
                        vmode     = NVP1918_VMODE_PAL_720H_2CH;
                        pixel_clk = 36000000;
                        break;
                    default: /* 144MHz */
                        vmode     = NVP1918_VMODE_PAL_720H_4CH;
                        pixel_clk = 144000000;
                        break;
                }
                break;
            case VCAP_INPUT_NORM_PAL_960H:
                ...
                switch(mode[id]) {
                    case 0:  /* 36MHz */
                        vmode     = NVP1918_VMODE_PAL_960H_1CH;
                        pixel_clk = 36000000;
                        break;
                    case 1:  /* 72MHz ==> 36MHz Clock with 72MHz Data */
                        vmode     = NVP1918_VMODE_PAL_960H_2CH;
                        pixel_clk = 36000000;
                        break;
                    default: /* 144MHz */
                        vmode     = NVP1918_VMODE_PAL_960H_4CH;
                        pixel_clk = 144000000;
                        break;
                }
                break;
            case VCAP_INPUT_NORM_NTSC_960H:
                ...
                switch(mode[id]) {
                    case 0:  /* 36MHz */
                        vmode     = NVP1918_VMODE_NTSC_960H_1CH;
                        pixel_clk = 36000000;
                        break;
                    case 1:  /* 72MHz ==> 36MHz Clock with 72MHz Data */
                        vmode     = NVP1918_VMODE_NTSC_960H_2CH;
                        pixel_clk = 36000000;
                        break;
                    default: /* 144MHz */
                        vmode     = NVP1918_VMODE_NTSC_960H_4CH;
                        pixel_clk = 144000000;
                        break;
                }
                break;
            default: /* NTSC */
                ...
                switch(mode[id]) {
                    case 0:  /* 36MHz */
                        vmode     = NVP1918_VMODE_NTSC_720H_1CH;
                        pixel_clk = 36000000;
                        break;
                    case 1:  /* 72MHz ==> 36MHz Clock with 72MHz Data */
                        vmode     = NVP1918_VMODE_NTSC_720H_2CH;
                        pixel_clk = 36000000;
                        break;
                    default: /* 144MHz */
                        vmode     = NVP1918_VMODE_NTSC_720H_4CH;
                        pixel_clk = 144000000;
                        break;
                }
                break;
        ...
        /* Update device norm */
        nvp1918_dev[id].norm  = norm;
        nvp1918_dev[id].vmode = vmode;
        for(i=0; i<DEV_VPORT_MAX; i++) {
            pinput = nvp1918_dev[id].port[i];
    ...
    if(ret < 0)
        vcap_err("NVP1918#%d init failed!\n", id);

    return ret;
}

static int nvp1918_proc_norm_show(struct seq_file *sfile, void *v)
{
...
static ssize_t nvp1918_proc_norm_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    int  ret = 0;
    int  norm_value;
    char value_str[8] = {'\0'};
    ...
    if((norm_value <= 3) && (norm_value != pdev->norm)) {
        ret = nvp1918_device_init(pdev->index, norm_value);
        if(ret < 0) {
            ret = -EFAULT;
            goto end;
        }
...
static int nvp1918_proc_timeout_show(struct seq_file *sfile, void *v)
{
    int i;
    u32 time_ms = 0;
    struct nvp1918_dev_t *pdev = (struct nvp1918_dev_t *)sfile->private;
...
static ssize_t nvp1918_proc_timeout_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    int  i, time_value;
    char value_str[32] = {'\0'};
    struct seq_file *sfile = (struct seq_file *)file->private_data;
...
static int nvp1918_proc_norm_open(struct inode *inode, struct file *file)
{
    return single_open(file, nvp1918_proc_norm_show, PDE(inode)->data);
}

static int nvp1918_proc_timeout_open(struct inode *inode, struct file *file)
{
    return single_open(file, nvp1918_proc_timeout_show, PDE(inode)->data);
}

static struct file_operations nvp1918_proc_norm_ops = {
    .owner  = THIS_MODULE,
    .open   = nvp1918_proc_norm_open,
    .write  = nvp1918_proc_norm_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations nvp1918_proc_timeout_ops = {
    .owner  = THIS_MODULE,
    .open   = nvp1918_proc_timeout_open,
    .write  = nvp1918_proc_timeout_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};
...
static void nvp1918_proc_remove(int id)
{
    if(nvp1918_proc_root[id]) {
        if(nvp1918_proc_timeout[id])
            vcap_input_proc_remove_entry(nvp1918_proc_root[id], nvp1918_proc_timeout[id]);

        if(nvp1918_proc_norm[id])
            vcap_input_proc_remove_entry(nvp1918_proc_root[id], nvp1918_proc_norm[id]);

        vcap_input_proc_remove_entry(NULL, nvp1918_proc_root[id]);
    }
}

static int nvp1918_proc_init(struct nvp1918_dev_t *pdev)
{
    int ret = 0;
    int id  = pdev->index;
    char name[32];
    ...
    /* root */
    sprintf(name, "nvp1918.%d", id);
    nvp1918_proc_root[id] = vcap_input_proc_create_entry(name, S_IFDIR|S_IRUGO|S_IXUGO, NULL);
    if(!nvp1918_proc_root[id]) {
    ...
    /* norm */
    nvp1918_proc_norm[id] = vcap_input_proc_create_entry("norm", S_IRUGO|S_IXUGO, nvp1918_proc_root[id]);
    if(!nvp1918_proc_norm[id]) {
        vcap_err("create proc node '%s/norm' failed!\n", name);
        ret = -EINVAL;
        goto err;
    }
    nvp1918_proc_norm[id]->proc_fops = &nvp1918_proc_norm_ops;
    nvp1918_proc_norm[id]->data      = (void *)pdev;
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,30))
    nvp1918_proc_norm[id]->owner     = THIS_MODULE;
#endif

    /* timeout */
    nvp1918_proc_timeout[id] = vcap_input_proc_create_entry("timeout", S_IRUGO|S_IXUGO, nvp1918_proc_root[id]);
    if(!nvp1918_proc_timeout[id]) {
        vcap_err("create proc node '%s/timeout' failed!\n", name);
        ret = -EINVAL;
        goto err;
    }
    nvp1918_proc_timeout[id]->proc_fops = &nvp1918_proc_timeout_ops;
    nvp1918_proc_timeout[id]->data      = (void *)pdev;
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,30))
    nvp1918_proc_timeout[id]->owner     = THIS_MODULE;
#endif
...
static int nvp1918_vlos_notify_handler(int id, int vin, int vlos)
{
    int i, j, vch;
    struct nvp1918_dev_t *pdev = &nvp1918_dev[id];
...
static int nvp1918_vfmt_notify_handler(int id, int vmode)
{
    ...
    if(pdev->vmode != vmode) {
        switch(vmode) {
            case NVP1918_VMODE_NTSC_720H_1CH:
            case NVP1918_VMODE_NTSC_720H_2CH:
            case NVP1918_VMODE_NTSC_720H_4CH:
                input_norm = VCAP_INPUT_NORM_NTSC;
                break;

            case NVP1918_VMODE_NTSC_960H_1CH:
            case NVP1918_VMODE_NTSC_960H_2CH:
            case NVP1918_VMODE_NTSC_960H_4CH:
                input_norm = VCAP_INPUT_NORM_NTSC_960H;
                break;

            case NVP1918_VMODE_PAL_720H_1CH:
            case NVP1918_VMODE_PAL_720H_2CH:
            case NVP1918_VMODE_PAL_720H_4CH:
                input_norm = VCAP_INPUT_NORM_PAL;
                break;
            case NVP1918_VMODE_PAL_960H_1CH:
            case NVP1918_VMODE_PAL_960H_2CH:
        ...
        ret = nvp1918_device_init(id, input_norm);
        if(ret == 0) {
            for(i=0; i<DEV_VPORT_MAX; i++) {
                if(!pdev->port[i])
                    continue;
...
static int __init nvp1918_input_init(void)
{
    int i, j;
    int dev_num;
    struct vcap_input_dev_t *pinput;
    ...
    /* clear buffer */
    memset(nvp1918_dev, 0, sizeof(struct nvp1918_dev_t)*DEV_MAX);

    /* Get NVP1918 Device Number */
    dev_num = nvp1918_get_device_num();

    for(i=0; i<DEV_MAX; i++) {
        nvp1918_dev[i].index = i;
        nvp1918_dev[i].norm  = -1;  ///< init value
        ...
        if(vport[i]) {
            ...
            for(j=0; j<DEV_VPORT_MAX; j++) {
                if((vport[i]>>(4*j)) & 0xf) {   ///< VPort# Connect to X_CAP#
                    pinput = nvp1918_dev[i].port[j] = kzalloc(sizeof(struct vcap_input_dev_t), GFP_KERNEL);
                    if(pinput == NULL) {
                        vcap_err("NVP1918#%d.%d allocate vcap_input_dev_t failed\n", i, j);
                        ret = -ENOMEM;
                        goto err;
                    }

                    /* input device name */
                    snprintf(pinput->name, VCAP_INPUT_NAME_SIZE-1, "nvp1918.%d.%d", i, j);
                    ...
                    pinput->init        = nvp1918_device_init;
                    pinput->norm.mode   = -1;                            ///< init value
                    pinput->module_get  = nvp1918_module_get;
                    pinput->module_put  = nvp1918_module_put;

                    switch(mode[i]) {
                        case 0:  /* 36MHz */
                            pinput->mode     = VCAP_INPUT_MODE_BYPASS;
                            pinput->ch_id[0] = nvp1918_get_vch_id(i, j, 0);
                            break;
                        case 1:  /* 72MHz ==> 36MHz Clock with 72MHz Data */
                            pinput->mode     = VCAP_INPUT_MODE_2CH_BYTE_INTERLEAVE;
                            pinput->ch_id[0] = nvp1918_get_vch_id(i, j, 0);
                            pinput->ch_id[2] = nvp1918_get_vch_id(i, j, 1);
                            break;
                        default: /* 144MHz */
                            pinput->mode     = VCAP_INPUT_MODE_4CH_BYTE_INTERLEAVE;
                            pinput->ch_id[0] = nvp1918_get_vch_id(i, j, 0);
                            pinput->ch_id[1] = nvp1918_get_vch_id(i, j, 1);
                            pinput->ch_id[2] = nvp1918_get_vch_id(i, j, 2);
                            pinput->ch_id[3] = nvp1918_get_vch_id(i, j, 3);
                            break;
                    }
                    ...
                    if(ret < 0) {
                        vcap_err("register NVP1918#%d.%d input device failed\n", i, j);
                        goto err;
                    }
                }
            }
            ...
            /* device proc init */
            ret = nvp1918_proc_init(&nvp1918_dev[i]);
            if(ret < 0)
                goto err;

            /* device init */
            ret = nvp1918_device_init(i, norm[i]);
            if(ret < 0)
                goto err;

            /* register nvp1918 video loss notify handler */
            nvp1918_notify_vlos_register(i, nvp1918_vlos_notify_handler);

            /* register nvp1918 video format notify handler */
            nvp1918_notify_vfmt_register(i, nvp1918_vfmt_notify_handler);

            vcap_info("Register NVP1918#%d Input Device\n", i);
        }
    }
...
err:
    for(i=0; i<DEV_MAX; i++) {
        nvp1918_notify_vlos_deregister(i);
        nvp1918_notify_vfmt_deregister(i);
        nvp1918_proc_remove(i);
        for(j=0; j<DEV_VPORT_MAX; j++) {
            if(nvp1918_dev[i].port[j]) {
                vcap_input_device_unregister(nvp1918_dev[i].port[j]);
                kfree(nvp1918_dev[i].port[j]);
            }
        }
    }
    return ret;
...
static void __exit nvp1918_input_exit(void)
{
    int i, j;
...
module_init(nvp1918_input_init);
module_exit(nvp1918_input_exit);

MODULE_DESCRIPTION("Grain Media Video Capture300 NVP1918 Input Driver");
