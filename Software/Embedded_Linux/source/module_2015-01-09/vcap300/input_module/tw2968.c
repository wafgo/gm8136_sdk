/**
 * @file tw2968.c
 * VCAP300 TW2968 Input Device Driver
 * Copyright (C) 2012 GM Corp. (http://www.grain-media.com)
 *
 * $Revision: 1.13 $
 * $Date: 2014/07/18 05:57:25 $
 *
 * ChangeLog:
 *  $Log: tw2968.c,v $
 *  Revision 1.13  2014/07/18 05:57:25  shawn_hu
 ...
 *
 *  Revision 1.4  2013/07/25 10:16:26  jerson_l
 *  1. change header file include path
 *
 *  Revision 1.3  2013/07/11 03:36:22  jerson_l
 *  1. modify video_set_mode control api
 *
 *  Revision 1.2  2013/03/19 06:08:56  jerson_l
 *  1. modify default field order to even field first
 *
 *  Revision 1.1  2013/02/20 08:55:51  jerson_l
 *  1. support tw2968
 *
 */
...
#define DEV_MAX                 TW2968_DEV_MAX
#define DEV_VPORT_MAX           TW2968_DEV_VPORT_MAX            ///< VD1, VD2, VD3, VD4
#define DEV_CH_MAX              TW2968_DEV_CH_MAX
...
static struct tw2968_dev_t    tw2968_dev[DEV_MAX];
static struct proc_dir_entry *tw2968_proc_root[DEV_MAX]    = {[0 ... (DEV_MAX - 1)] = NULL};
static struct proc_dir_entry *tw2968_proc_norm[DEV_MAX]    = {[0 ... (DEV_MAX - 1)] = NULL};
static struct proc_dir_entry *tw2968_proc_timeout[DEV_MAX] = {[0 ... (DEV_MAX - 1)] = NULL};

/****************************************************************************************
 *  TW2968 VPort and X_CAP# Port Mapping for GM8210/GM8287 Socket Board
 *
 *  TW2968#0(0x50) --->VD1 ---- [VIN#0,1,2,3] 108/144MHz ---> X_CAP#1
 *                 --->VD2 ---- [VIN#4,5,6,7] 108/144MHz ---> X_CAP#0
 *                 --->VD3 ---- X
 *                 --->VD4 ---- X
 *
 *  TW2968#1(0x52) --->VD1 ---- [VIN#0,1,2,3] 108/144MHz ---> X_CAP#3
 *                 --->VD2 ---- [VIN#4,5,6,7] 108/144MHz ---> X_CAP#2
 *                 --->VD3 ---- X
 *                 --->VD4 ---- X
 *
 *  TW2968#1(0x54) --->VD1 ---- [VIN#0,1,2,3] 108/144MHz ---> X_CAP#5
 *                 --->VD2 ---- [VIN#4,5,6,7] 108/144MHz ---> X_CAP#4
 *                 --->VD3 ---- X
 *                 --->VD4 ---- X
 *
 *  TW2968#1(0x56) --->VD1 ---- [VIN#0,1,2,3] 108/144MHz ---> X_CAP#7
 *                 --->VD2 ---- [VIN#4,5,6,7] 108/144MHz ---> X_CAP#6
 *                 --->VD3 ---- X
 *                 --->VD4 ---- X
 *****************************************************************************************/

static int tw2968_module_get(void)
{
    return try_module_get(THIS_MODULE);
}

static void tw2968_module_put(void)
{
    module_put(THIS_MODULE);
}

static int tw2968_device_init(int id, int norm)
{
    int i, ret = 0;
    ...
    if((id >= DEV_MAX) || (id >= tw2968_get_device_num()))
        return -1;

    if(tw2968_dev[id].norm != norm) {
        switch(norm) {
            case VCAP_INPUT_NORM_PAL:
                ...
                switch(mode[id]) {
                    case 0:  /* 27MHz */
                        vmode     = TW2968_VMODE_PAL_720H_1CH;
                        pixel_clk = 27000000;
                        break;
                    case 1:  /* 54MHz ==> 27MHz Clock with 54MHz Data */
                        vmode     = TW2968_VMODE_PAL_720H_2CH;
                        pixel_clk = 27000000;
                        break;
                    default: /* 108MHz */
                        vmode     = TW2968_VMODE_PAL_720H_4CH;
                        pixel_clk = 108000000;
                        break;
                }
                break;
            case VCAP_INPUT_NORM_PAL_960H:
                ...
                switch(mode[id]) {
                    case 0:  /* 36MHz */
                        vmode     = TW2968_VMODE_PAL_960H_1CH;
                        pixel_clk = 36000000;
                        break;
                    case 1:  /* 72MHz ==> 36MHz Clock with 72MHz Data  */
                        vmode     = TW2968_VMODE_PAL_960H_2CH;
                        pixel_clk = 36000000;
                        break;
                    default: /* 144MHz */
                        vmode     = TW2968_VMODE_PAL_960H_4CH;
                        pixel_clk = 144000000;
                        break;
                }
                break;
            case VCAP_INPUT_NORM_NTSC_960H:
                ...
                switch(mode[id]) {
                    case 0:  /* 36MHz */
                        vmode     = TW2968_VMODE_NTSC_960H_1CH;
                        pixel_clk = 36000000;
                        break;
                    case 1:  /* 72MHz ==> 36MHz Clock with 72MHz Data  */
                        vmode     = TW2968_VMODE_NTSC_960H_2CH;
                        pixel_clk = 36000000;
                        break;
                    default: /* 144MHz */
                        vmode     = TW2968_VMODE_NTSC_960H_4CH;
                        pixel_clk = 144000000;
                        break;
                }
                break;
            default: /* NTSC */
                ...
                switch(mode[id]) {
                    case 0:  /* 27MHz */
                        vmode     = TW2968_VMODE_NTSC_720H_1CH;
                        pixel_clk = 27000000;
                        break;
                    case 1:  /* 54MHz ==> 27MHz Clock with 54MHz Data  */
                        vmode     = TW2968_VMODE_NTSC_720H_2CH;
                        pixel_clk = 27000000;
                        break;
                    default: /* 108MHz */
                        vmode     = TW2968_VMODE_NTSC_720H_4CH;
                        pixel_clk = 108000000;
                        break;
                }
                break;
        ...
        /* Update device norm */
        tw2968_dev[id].norm = norm;
        tw2968_dev[id].vmode = vmode;
        for(i=0; i<DEV_VPORT_MAX; i++) {
            pinput = tw2968_dev[id].port[i];
    ...
    if(ret < 0)
        vcap_err("TW2968#%d init failed!\n", id);

    return ret;
}

static int tw2968_proc_norm_show(struct seq_file *sfile, void *v)
{
...
static int tw2968_proc_timeout_show(struct seq_file *sfile, void *v)
{
    int i;
    u32 time_ms = 0;
    struct tw2968_dev_t *pdev = (struct tw2968_dev_t *)sfile->private;
...
static ssize_t tw2968_proc_norm_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    int  ret = 0;
    int  norm_value;
    char value_str[8] = {'\0'};
    ...
    if((norm_value <= 3) && (norm_value != pdev->norm)) {
        ret = tw2968_device_init(pdev->index, norm_value);
        if(ret < 0) {
            ret = -EFAULT;
            goto end;
        }
...
static ssize_t tw2968_proc_timeout_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    int  i, time_value;
    char value_str[32] = {'\0'};
    struct seq_file *sfile = (struct seq_file *)file->private_data;
...
static int tw2968_proc_norm_open(struct inode *inode, struct file *file)
{
    return single_open(file, tw2968_proc_norm_show, PDE(inode)->data);
}

static int tw2968_proc_timeout_open(struct inode *inode, struct file *file)
{
    return single_open(file, tw2968_proc_timeout_show, PDE(inode)->data);
}

static struct file_operations tw2968_proc_norm_ops = {
    .owner  = THIS_MODULE,
    .open   = tw2968_proc_norm_open,
    .write  = tw2968_proc_norm_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations tw2968_proc_timeout_ops = {
    .owner  = THIS_MODULE,
    .open   = tw2968_proc_timeout_open,
    .write  = tw2968_proc_timeout_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};
...
static void tw2968_proc_remove(int id)
{
    if(tw2968_proc_root[id]) {
        if(tw2968_proc_timeout[id])
            vcap_input_proc_remove_entry(tw2968_proc_root[id], tw2968_proc_timeout[id]);

        if(tw2968_proc_norm[id])
            vcap_input_proc_remove_entry(tw2968_proc_root[id], tw2968_proc_norm[id]);

        vcap_input_proc_remove_entry(NULL, tw2968_proc_root[id]);
    }
}

static int tw2968_proc_init(struct tw2968_dev_t *pdev)
{
    int ret = 0;
    int id  = pdev->index;
    char name[32];
    ...
    /* root */
    sprintf(name, "tw2968.%d", id);
    tw2968_proc_root[id] = vcap_input_proc_create_entry(name, S_IFDIR|S_IRUGO|S_IXUGO, NULL);
    if(!tw2968_proc_root[id]) {
    ...
    /* norm */
    tw2968_proc_norm[id] = vcap_input_proc_create_entry("norm", S_IRUGO|S_IXUGO, tw2968_proc_root[id]);
    if(!tw2968_proc_norm[id]) {
        vcap_err("create proc node '%s/norm' failed!\n", name);
        ret = -EINVAL;
        goto err;
    }
    tw2968_proc_norm[id]->proc_fops = &tw2968_proc_norm_ops;
    tw2968_proc_norm[id]->data      = (void *)pdev;
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,30))
    tw2968_proc_norm[id]->owner     = THIS_MODULE;
#endif

    /* timeout */
    tw2968_proc_timeout[id] = vcap_input_proc_create_entry("timeout", S_IRUGO|S_IXUGO, tw2968_proc_root[id]);
    if(!tw2968_proc_timeout[id]) {
        vcap_err("create proc node '%s/timeout' failed!\n", name);
        ret = -EINVAL;
        goto err;
    }
    tw2968_proc_timeout[id]->proc_fops = &tw2968_proc_timeout_ops;
    tw2968_proc_timeout[id]->data      = (void *)pdev;
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,30))
    tw2968_proc_timeout[id]->owner     = THIS_MODULE;
#endif
...
static int tw2968_vlos_notify_handler(int id, int vin, int vlos)
{
    int i, j, vch;
    struct tw2968_dev_t *pdev = &tw2968_dev[id];
...
static int tw2968_vfmt_notify_handler(int id, int vmode)
{
    ...
    if(pdev->vmode != vmode) {
        ...
        ret = tw2968_device_init(id, input_norm);
        if(ret == 0) {
            for(i=0; i<DEV_VPORT_MAX; i++) {
                if(!pdev->port[i])
                    continue;
...
static int __init tw2968_input_init(void)
{
    int i, j;
    int dev_num;
    struct vcap_input_dev_t *pinput;
    ...
    /* clear buffer */
    memset(tw2968_dev, 0, sizeof(struct tw2968_dev_t)*DEV_MAX);

    /* Get TW2968 Device Number */
    dev_num = tw2968_get_device_num();

    for(i=0; i<DEV_MAX; i++) {
        tw2968_dev[i].index = i;
        tw2968_dev[i].norm  = -1;  ///< init value
        ...
        if(vport[i]) {
            ...
            for(j=0; j<DEV_VPORT_MAX; j++) {
                if((vport[i]>>(4*j)) & 0xf) {   ///< VPort# Connect to X_CAP#
                    pinput = tw2968_dev[i].port[j] = kzalloc(sizeof(struct vcap_input_dev_t), GFP_KERNEL);
                    if(pinput == NULL) {
                        vcap_err("TW2968#%d.%d allocate vcap_input_dev_t failed\n", i, j);
                        ret = -ENOMEM;
                        goto err;
                    }

                    /* input device name */
                    snprintf(pinput->name, VCAP_INPUT_NAME_SIZE-1, "tw2968.%d.%d", i, j);
                    ...
                    pinput->init        = tw2968_device_init;
                    pinput->norm.mode   = -1;                            ///< init value
                    pinput->module_get  = tw2968_module_get;
                    pinput->module_put  = tw2968_module_put;

                    switch(mode[i]) {
                        case 0:  /* 27MHz/36MHz */
                            pinput->mode     = VCAP_INPUT_MODE_BYPASS;
                            pinput->ch_id[0] = tw2968_get_vch_id(i, j, 0);
                            break;
                        case 1:  /* 54MHz/72MHz ==> 27MHz/36MHz Clock with 54MHz/72MHz Data */
                            pinput->mode     = VCAP_INPUT_MODE_2CH_BYTE_INTERLEAVE;
                            pinput->ch_id[0] = tw2968_get_vch_id(i, j, 0);
                            pinput->ch_id[2] = tw2968_get_vch_id(i, j, 1);
                            break;
                        default: /* 108MHz/144MHz */
                            pinput->mode     = VCAP_INPUT_MODE_4CH_BYTE_INTERLEAVE;
                            pinput->ch_id[0] = tw2968_get_vch_id(i, j, 0);
                            pinput->ch_id[1] = tw2968_get_vch_id(i, j, 1);
                            pinput->ch_id[2] = tw2968_get_vch_id(i, j, 2);
                            pinput->ch_id[3] = tw2968_get_vch_id(i, j, 3);
                            break;
                    }
                    ...
                    if(ret < 0) {
                        vcap_err("register TW2968#%d.%d input device failed\n", i, j);
                        goto err;
                    }
                }
            }
            ...
            /* device proc init */
            ret = tw2968_proc_init(&tw2968_dev[i]);
            if(ret < 0)
                goto err;

            /* device init */
            ret = tw2968_device_init(i, norm[i]);
            if(ret < 0)
                goto err;

            /* register tw2968 video loss notify handler */
            tw2968_notify_vlos_register(i, tw2968_vlos_notify_handler);

            /* register tw2968 video format notify handler */
            tw2968_notify_vfmt_register(i, tw2968_vfmt_notify_handler);

            vcap_info("Register TW2968#%d Input Device\n", i);
        }
    }
...
err:
    for(i=0; i<DEV_MAX; i++) {
        tw2968_notify_vlos_deregister(i);
        tw2968_notify_vfmt_deregister(i);
        tw2968_proc_remove(i);
        for(j=0; j<DEV_VPORT_MAX; j++) {
            if(tw2968_dev[i].port[j]) {
                vcap_input_device_unregister(tw2968_dev[i].port[j]);
                kfree(tw2968_dev[i].port[j]);
            }
        }
    }
    return ret;
...
static void __exit tw2968_input_exit(void)
{
    int i, j;
...
module_init(tw2968_input_init);
module_exit(tw2968_input_exit);

MODULE_DESCRIPTION("Grain Media Video Capture300 TW2968 Input Driver");
