 ...
 * =============================================================
 * file  : config.c
 * brief : config parser for vg verify configuration
 * =============================================================
 ***************************************************************/
#include <stdio.h>
#include <stdlib.h>
...
static int getkeyname(const char *line, char *keyname, unsigned int keylen)
{
    ...
    for (end = start; line[end] != '=' && end < strlen(line); end++) {
        if (!isalnum(line[end]) && line[end] != '_' && line[end] != ' ' && line[end] != '\t')
            return 0;
    }
...
int getconfiguint(const char *section, const char *keyname, unsigned int *keyvalue, const char *filename)
{
    int rs;
    char kev[12];
