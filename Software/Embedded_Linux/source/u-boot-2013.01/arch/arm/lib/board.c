...
extern void init_sys_freq(void);
...
//#endif
...
#ifdef CONFIG_PLATFORM_GM8210
    {
        ...
        extern void slave_gm8210_boot(void);

        /* record the quantity of slave 8210 device in private address */
        tmp = readl(CONFIG_PMU_BASE + 0xBC);
        tmp &= ~(0xFF << 24);
        tmp |= (slave_gm8210_quantity << 24);
        writel(tmp, CONFIG_PMU_BASE + 0xBC);
