...
//#define TIMER_LOAD_VAL	135000000  //APB clock u32PMU_ReadPCLK
#define CFG_HZ              1000

extern unsigned int u32PMU_ReadPCLK(void);
...
ulong get_timer_masked (void)
{
	...
	if(now > lastdec) {
		/* Must have wrapped */
		total_count += lastdec + u32PMU_ReadPCLK() + 1 - now;
	} else {
...
int timer_init(void)
{
	...
	writel(u32PMU_ReadPCLK(), CONFIG_TMR_BASE);
	/* Set timer to be
	 *	enabled			1
	 *	periodic		1
	 *	no interrupts		0
...
//	writel(CONFIG_TMR_BASE + 8) = 0x000000C2;
	writel(u32PMU_ReadPCLK(), CONFIG_TMR_BASE + 4);//0xffffffff;
	...
	div_timer	= (unsigned long long)(u32PMU_ReadPCLK() / CFG_HZ); /* each u-boot tick need how much timer tick */
	div_timer /= div_clock;
...
//	tmo	= usec * CFG_HZ;
//	tmo /= (1000000L);
	tmo	= usec * (u32PMU_ReadPCLK() /1000000L);  /* need how much timer ticks */
