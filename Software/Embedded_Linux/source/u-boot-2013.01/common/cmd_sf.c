...
static int spi_load_ep_image = 0;
#endif
...
static int do_spi_flash_read_write(int argc, char * const argv[])
{
    ...
    if (partno >= 0) {
        offset = spi_sys_hdr->image[partno].addr;
        len = spi_sys_hdr->image[partno].size;
        goto step1;
    }

    if (strcmp(argv[2], "x") == 0){        
        /*printf("image at flash addr = 0x%x, size = 0x%x, load to DRAM %s\n", 
        	spi_sys_hdr->image[3].addr, spi_sys_hdr->image[3].size, argv[1]);*/
        offset = spi_sys_hdr->image[3].addr;
        len = spi_sys_hdr->image[3].size;
        goto step1;
    }
    
    if (strcmp(argv[2], "y") == 0){        
        /*printf("image at flash addr = 0x%x, size = 0x%x, load to DRAM %s\n", 
        	spi_sys_hdr->image[2].addr, spi_sys_hdr->image[2].size, argv[1]);*/
        offset = spi_sys_hdr->image[2].addr;
        len = spi_sys_hdr->image[2].size;
        goto step1;
    }
    
    if (strcmp(argv[2], "z") == 0){        
        /*printf("image at flash addr = 0x%x, size = 0x%x, load to DRAM %s\n", 
        	spi_sys_hdr->image[1].addr, spi_sys_hdr->image[1].size, argv[1]);*/
        offset = spi_sys_hdr->image[1].addr;
        len = spi_sys_hdr->image[1].size;
        goto step1;
    }
#ifdef CONFIG_PLATFORM_GM8210
    if(slave_gm8210_quantity){
        if (strcmp(argv[2], "w") == 0){
            ...
            /*printf("image EP at flash addr = 0x%x, size = 0x%x, load to DRAM %s\n",
            	spi_sys_hdr->image[2].addr, spi_sys_hdr->image[2].size, argv[1]);*/
            offset = spi_sys_hdr->image[2].addr;
            if(spi_sys_hdr->image[2].size > 0x500000)
                len = spi_sys_hdr->image[2].size;
            else
                len = 0x500000;//for kernel + ramdisk size
...
step1:
    ...
    if(slave_gm8210_quantity){
        ...
        if ((strcmp(argv[0], "read") == 0) && spi_load_ep_image) {
            spi_load_ep_image = 0;        
    
            sprintf(cmd_load_image, "loadsl 0x%08x 0x%x", (uint) addr, len);
            printf("%s:\n", cmd_load_image);
