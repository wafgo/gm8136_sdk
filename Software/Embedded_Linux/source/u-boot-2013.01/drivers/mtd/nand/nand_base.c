...
extern void ftnandc_set_bbt(int block_num);
/**
			      ...
			      int page)
{
    ...
    //printf("erase, page=%d\n",page);
    printf("<markbad>");
#if defined(CONFIG_NAND024V2_GM) || defined(CONFIG_NAND_GM)   
    chip->cmdfunc(mtd, NAND_CMD_ERASE1, 0x00, page);

    chip->cmdfunc(mtd, NAND_CMD_BI_TABLE, 0x00, 0);
    chip->ecc.write_page(mtd, chip, (const uint8_t *)nand_sys_hdr);
#endif
	chip->cmdfunc(mtd, NAND_CMD_BI_TABLE, 0x00, page);
						  ...
						  const struct nand_flash_dev *type)
{
	...
	for (i = 0; i < 4; i++)//we default read 4 bytes ID number, we can search ID_num string 
	...
	*maf_id = id_data[0];//chip->read_byte(mtd);
	*dev_id = id_data[1];//chip->read_byte(mtd);
#if 0
